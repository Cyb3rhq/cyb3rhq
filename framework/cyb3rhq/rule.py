...
import cyb3rhq.core.configuration as configuration
...
from cyb3rhq.core.rule import check_status, load_rules_from_file, format_rule_decoder_file, REQUIRED_FIELDS, \
    RULE_REQUIREMENTS, SORT_FIELDS, RULE_FIELDS, RULE_FILES_FIELDS, RULE_FILES_REQUIRED_FIELDS
from cyb3rhq.core.utils import process_array, safe_move, \
    validate_cyb3rhq_xml, upload_file, full_copy, to_relative_path
from cyb3rhq.core.logtest import validate_dummy_logtest
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsCyb3rhqResult(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=Cyb3rhqError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
    ...
    ------
    Cyb3rhqError(1200)
        Error reading rules from `CYB3RHQ_HOME/etc/ossec.conf`.
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(none_msg='No rule files were returned',
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsCyb3rhqResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=Cyb3rhqError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.CYB3RHQ_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.CYB3RHQ_PATH, rules[0]['relative_dirname'], filename))


def get_rule_file(filename: str = None, raw: bool = False,
                  relative_dirname: str = None) -> Union[str, AffectedItemsCyb3rhqResult]:
    ...
    Returns
    -------
    str or AffectedItemsCyb3rhqResult
        Content of the file. AffectedItemsCyb3rhqResult format if `raw=False`.
    """
    result = AffectedItemsCyb3rhqResult(none_msg='No rule was returned',
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=Cyb3rhqError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=Cyb3rhqError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=Cyb3rhqError(1414, extra_message=f"{filename}"))

    return result


def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, Cyb3rhqError]:
    ...
    Tuple (str, Cyb3rhqError)
        ...
        The second element of the tuple is a Cyb3rhqError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return Cyb3rhqError(1505).
            If relative_dirname is inside the default RULES_PATH return Cyb3rhqError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return Cyb3rhqError(1507).
    ...
    if not relative_dirname in ruleset_conf['rule_dir']:
        cyb3rhq_error = Cyb3rhqError(1209)
    elif commonpath([join(common.CYB3RHQ_PATH, relative_dirname), common.RULES_PATH]) == common.RULES_PATH:
        cyb3rhq_error = Cyb3rhqError(1210)
    elif not exists(join(common.CYB3RHQ_PATH, relative_dirname)):
        cyb3rhq_error = Cyb3rhqError(1211)
    return relative_dirname, cyb3rhq_error


@expose_resources(actions=['rules:update'], resources=['*:*:*'])
def upload_rule_file(filename: str, content: str, relative_dirname: str = None, 
                     overwrite: bool = False) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='Rule was successfully uploaded',
                                      none_msg='Could not upload rule')

    backup_file = ''
    try:
        relative_dirname, cyb3rhq_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.CYB3RHQ_PATH, relative_dirname, filename)
        if cyb3rhq_error:
            raise cyb3rhq_error
        if len(content) == 0:
            raise Cyb3rhqError(1112)

        validate_cyb3rhq_xml(content)

        # If file already exists and overwrite is False, raise exception
        if not overwrite and exists(full_path):
            raise Cyb3rhqError(1905)
        elif overwrite and exists(full_path):
            backup_file = f'{full_path}.backup'
            try:
                full_copy(full_path, backup_file)
            except IOError as exc:
                raise Cyb3rhqError(1019) from exc
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsCyb3rhqResult:
    """Delete a rule file.
    ...
    result = AffectedItemsCyb3rhqResult(all_msg='Rule was successfully deleted',
                                      none_msg='Could not delete rule')

    try:
        relative_dirname, cyb3rhq_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.CYB3RHQ_PATH, relative_dirname, file)
        if cyb3rhq_error:
            raise cyb3rhq_error
        ...
        if exists(full_path):
            try:
                ...
                raise Cyb3rhqError(1907) from exc
        else:
