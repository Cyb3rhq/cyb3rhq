...
from cyb3rhq.core import common, configuration
from cyb3rhq.core.cluster.cluster import get_node
from cyb3rhq.core.cluster.utils import manager_restart, read_cluster_config
from cyb3rhq.core.configuration import get_ossec_conf, write_ossec_conf
from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqInternalError
from cyb3rhq.core.manager import status, get_api_conf, get_update_information_template, get_ossec_logs, \
...
from cyb3rhq.core.utils import process_array, safe_move, validate_cyb3rhq_xml, full_copy
...
def get_status() -> AffectedItemsCyb3rhqResult:
    """Wrapper for status().
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Processes status was successfully read"
...
def ossec_log(level: str = None, tag: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
              ...
              q: str = '', select: str = None, distinct: bool = False) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Logs were successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def ossec_log_summary() -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Log was successfully summarized"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def get_api_config() -> AffectedItemsCyb3rhqResult:
    ...
    """
    result = AffectedItemsCyb3rhqResult(**_get_config_default_result_kwargs)
...
def restart() -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def validation() -> AffectedItemsCyb3rhqResult:
    """Check if Cyb3rhq configuration is OK.
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(**_validation_default_result_kwargs)
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_config(component: str = None, config: str = None) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Active configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def read_ossec_conf(section: str = None, field: str = None, raw: bool = False,
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_basic_info() -> AffectedItemsCyb3rhqResult:
    """Wrapper for Cyb3rhq().to_dict
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Basic information was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read basic information in some nodes',
    ...
    try:
        result.affected_items.append(Cyb3rhq().to_dict())
    except Cyb3rhqError as e:
        result.add_failed_item(id_=node_id, error=e)
    result.total_affected_items = len(result.affected_items)
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:update_config"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def update_ossec_conf(new_conf: str = None) -> AffectedItemsCyb3rhqResult:
    """Replace cyb3rhq configuration (ossec.conf) with the provided configuration.
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg=f"Configuration was successfully updated"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
    ...
    try:
        ...
        if not new_conf:
            raise Cyb3rhqError(1125)

        # Check if the configuration is valid
        validate_cyb3rhq_xml(new_conf, config_file=True)
...
def get_update_information(installation_uid: str, update_information: dict) -> Cyb3rhqResult:
    """Process update information into a cyb3rhq result.

    Parameters
    ----------
    installation_uid : str
        Cyb3rhq UID to include in the result.
    ...
    if not update_information:
        ...
        return Cyb3rhqResult({'data': get_update_information_template(uuid=installation_uid, update_check=False)})
    ...
    if status_code != 200:
        ...
        raise Cyb3rhqInternalError(2100, extra_message=extra_message)

    update_information.pop('message', None)

    return Cyb3rhqResult({'data': update_information})
