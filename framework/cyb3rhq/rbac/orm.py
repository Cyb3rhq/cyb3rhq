...
from cyb3rhq.core.common import cyb3rhq_uid, cyb3rhq_gid, DEFAULT_RBAC_RESOURCES
from cyb3rhq.core.utils import get_utc_now, safe_move
from cyb3rhq.rbac.utils import clear_cache

logger = logging.getLogger("cyb3rhq-api")

# Max reserved ID value
CYB3RHQ_USER_ID = 1
CYB3RHQ_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (CYB3RHQ_USER_ID, CYB3RHQ_WUI_USER_ID):
        ...
        if from_id == CYB3RHQ_USER_ID and to_id == CYB3RHQ_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Cyb3rhq ownership and permissions.
        ...
        """
        chown(database, cyb3rhq_uid(), cyb3rhq_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=CYB3RHQ_USER_ID,
                                        to_id=CYB3RHQ_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(cyb3rhq_uid(), cyb3rhq_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current Cyb3rhq RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
