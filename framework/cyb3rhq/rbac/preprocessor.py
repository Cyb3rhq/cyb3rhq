...
from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqPermissionError
from cyb3rhq.core.results import Cyb3rhqResult
from cyb3rhq.rbac.auth_context import RBAChecker, get_policies_from_roles
from cyb3rhq.rbac.orm import AuthenticationManager
        ...
        for action in policy['actions']:
            ...
            for resource in policy['resources']:
                if not re.match(resource_regex, resource):
                    raise Cyb3rhqError(4500)
                resource_type = PreProcessor.is_combination(resource)
                if len(resource_type[1]) > 2:
                    raise Cyb3rhqError(4500, extra_remediation="The maximum length for permission combinations is two")
...
def get_permissions(user_id: int = None, auth_context: Union[dict, str] = None) -> Cyb3rhqResult:
