...
from cyb3rhq.core.cluster.control import get_health, get_nodes, get_node_ruleset_integrity
from cyb3rhq.core.cluster.utils import get_cluster_status, read_cluster_config, read_config
...
from cyb3rhq.rbac.decorators import expose_resources, async_list_handler
...
def read_config_wrapper() -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='All selected information was returned',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_node_wrapper() -> AffectedItemsCyb3rhqResult:
...
def get_status_json() -> Cyb3rhqResult:
    """Return the cluster status.

    Returns
    -------
    Cyb3rhqResult
        Cyb3rhqResult object with the cluster status.
    """
    return Cyb3rhqResult({'data': get_cluster_status()})
...
async def get_health_nodes(lc: local_client.LocalClient,
                           filter_node: Union[str, list] = None) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='All selected nodes healthcheck information was returned',
...
async def get_nodes_info(lc: local_client.LocalClient, filter_node: Union[str, list] = None,
                         **kwargs: dict) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='All selected nodes information was returned',
    ...
    for node in non_existent_nodes:
        result.add_failed_item(id_=node, error=Cyb3rhqResourceNotFound(1730))
...
async def get_ruleset_sync_status(master_md5: dict = None):
    ...
    """
    result = AffectedItemsCyb3rhqResult(all_msg="Nodes ruleset synchronization status was successfully read",
