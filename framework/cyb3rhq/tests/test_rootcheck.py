...
with patch('wazuh.common.cyb3rhq_uid'):
    with patch('wazuh.common.cyb3rhq_gid'):
        ...
        from cyb3rhq import rootcheck
        from cyb3rhq.core.rootcheck import Cyb3rhqDBQueryRootcheck
        from cyb3rhq.core.tests.test_rootcheck import InitRootcheck, send_msg_to_wdb, remove_db, \
...
@patch('cyb3rhq.rootcheck.Cyb3rhqDBQueryAgents.__exit__')
@patch('cyb3rhq.rootcheck.Cyb3rhqDBQueryAgents.__init__', return_value=None)
...
def test_rootcheck_run(close_mock, send_mock, connect_mock, agent_init__mock, agent_exit__mock,
    ...
    agent_list : list
        List of agent IDs.
    failed_items : list
        List with the Cyb3rhqDBQueryAgents response.
    status_list : list
        List of agent statuses.
    expected_result : list
        List of dicts with expected results for every test.
    """
    with patch('cyb3rhq.rootcheck.Cyb3rhqDBQueryAgents.run', return_value=failed_items[0]):
        ...
        assert isinstance(result, rootcheck.AffectedItemsCyb3rhqResult)
...
@pytest.mark.parametrize('agent_list, expected_affected_items, expected_calls, wdb_side_effect', [
    (['002'], [], [call('agent 002 rootcheck delete')], Cyb3rhqError(2004)),
    (['000', '001', '003'], ['000', '001'], [call('agent 000 rootcheck delete'), call('agent 001 rootcheck delete')],
     [None, None])
])
@patch('cyb3rhq.rootcheck.get_agents_info', return_value={'000', '001', '002'})
...
def test_clear(mock_connect, mock_info, agent_list, expected_affected_items, expected_calls, wdb_side_effect):
    ...
    expected_calls : list
        List of expected calls to the mocked Cyb3rhqDBConnection._send function.
    wdb_side_effect : Cyb3rhqError or list
        Side effect used in the mocked Cyb3rhqDBConnection._send function.
    """
    with patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=wdb_side_effect) as mock_wdbconn:
...
def test_get_rootcheck_agent(mock_connect, mock_send, mock_info, mock_exists, limit):
    ...
    for item in result['affected_items']:
        for key in item.keys():
            assert key in Cyb3rhqDBQueryRootcheck.fields
...
def test_get_rootcheck_agent_select(mock_connect, mock_send, mock_info, mock_exists, select):
    ...
    result = rootcheck.get_rootcheck_agent(agent_list=['001'], select=select, filters={'status': 'all'}).render()[
        'data']
    select = select if select else list(Cyb3rhqDBQueryRootcheck.fields.keys())
