...
import pytest
from cyb3rhq.core.common import USER_DECODERS_PATH


with patch('cyb3rhq.core.common.getgrnam'):
    with patch('cyb3rhq.core.common.getpwnam'):
...
def mock_cyb3rhq_paths():
    """Mock cyb3rhq paths."""
    with patch('cyb3rhq.core.common.CYB3RHQ_PATH', new=test_data_path):
        with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('cyb3rhq.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('cyb3rhq.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('cyb3rhq.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, cyb3rhq.core.common.CYB3RHQ_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'cyb3rhq', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'cyb3rhq', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'cyb3rhq', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'cyb3rhq', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, Cyb3rhqInternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_cyb3rhq_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename, 
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(cyb3rhq.core.common.CYB3RHQ_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_cyb3rhq_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsCyb3rhqResult
        assert isinstance(result, AffectedItemsCyb3rhqResult)
...
@patch('cyb3rhq.decoder.delete_decoder_file')
@patch('cyb3rhq.decoder.full_copy')
@patch('cyb3rhq.decoder.validate_cyb3rhq_xml')
@patch('cyb3rhq.decoder.upload_file')
@patch('cyb3rhq.decoder.remove')
@patch('cyb3rhq.decoder.safe_move')
@patch('cyb3rhq.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_cyb3rhq_paths,
    ...
    with patch('cyb3rhq.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('cyb3rhq.decoder.exists', return_value=overwrite):
            with patch('cyb3rhq.decoder.to_relative_path',
                    side_effect=lambda x: os.path.relpath(x, cyb3rhq.core.common.CYB3RHQ_PATH)):
            ...
            if overwrite:
                full_path = os.path.join(wazuh.common.CYB3RHQ_PATH, decoder_path)
...
@patch('cyb3rhq.decoder.delete_decoder_file', side_effect=Cyb3rhqError(1019))
@patch('cyb3rhq.decoder.upload_file')
@patch('cyb3rhq.decoder.safe_move')
...
def test_upload_file_ko(*_):
    ...
    with patch('cyb3rhq.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('cyb3rhq.decoder.exists'):
    ...
    # clean backup files
    search_pattern = os.path.join(cyb3rhq.core.common.CYB3RHQ_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('cyb3rhq.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsCyb3rhqResult when everything is correct
        with patch('cyb3rhq.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('cyb3rhq.decoder.exists', return_value=True):
        with patch('cyb3rhq.decoder.remove', side_effect=IOError()):
