...
from cyb3rhq.core.common import USER_RULES_PATH
import pytest
...
def mock_cyb3rhq_paths():
    with patch('cyb3rhq.core.common.RULES_PATH', new=os.path.join(parent_directory, core_data_path)):
        with patch('cyb3rhq.core.common.USER_RULES_PATH', new=os.path.join(parent_directory, tests_data_path)):
            with patch('cyb3rhq.core.common.CYB3RHQ_PATH', new=parent_directory):
                with patch('cyb3rhq.rule.to_relative_path', side_effect=lambda x: os.path.relpath(x, parent_directory)):
                    yield
...
@patch("cyb3rhq.rule.configuration.get_ossec_conf", return_value=rule_ossec_conf)
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(Cyb3rhqError, match='.* 1202 .*'):
            func(status=status)
    else:
...
@patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=rule_ossec_conf)
...
@patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=other_rule_ossec_conf)
...
def test_failed_get_rules():
    ...
    with pytest.raises(Cyb3rhqError, match=".* 1203 .*"):
...
def test_get_rule_file_path(filename, relative_dirname, result, mock_cyb3rhq_paths):
    """Test get_rule_file_path function."""
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        res = rule.get_rule_file_path(filename=filename, 
                                            relative_dirname=relative_dirname)
        assert res == os.path.join(cyb3rhq.core.common.CYB3RHQ_PATH, result) if result else not res
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsCyb3rhqResult)
...
@patch('cyb3rhq.rule.delete_rule_file')
@patch('cyb3rhq.rule.full_copy')
@patch('cyb3rhq.rule.upload_file')
@patch('cyb3rhq.rule.remove')
@patch('cyb3rhq.rule.safe_move')
@patch('cyb3rhq.rule.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_xml, mock_full_copy,
    ...
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('cyb3rhq.rule.validate_upload_delete_dir', return_value=ret_validation):
            with patch('cyb3rhq.rule.exists', return_value=overwrite):
                ...
                if overwrite:
                    full_path = os.path.join(wazuh.common.CYB3RHQ_PATH, rule_path)
...
@patch('cyb3rhq.rule.delete_rule_file', side_effect=Cyb3rhqError(1019))
@patch('cyb3rhq.rule.upload_file')
@patch('cyb3rhq.rule.safe_move')
...
def test_delete_rule_file(file, relative_dirname):
    ...
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        with patch('cyb3rhq.rule.exists', return_value=True):
            with patch('cyb3rhq.rule.remove'):
...
def test_delete_rule_file_ko():
    ...
    with patch('cyb3rhq.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('cyb3rhq.rule.remove', side_effect=IOError()):
