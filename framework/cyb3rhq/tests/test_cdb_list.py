...
from cyb3rhq.core.exception import Cyb3rhqInternalError
...
with patch('cyb3rhq.core.common.getgrnam'):
    with patch('cyb3rhq.core.common.getpwnam'):
        ...
        from cyb3rhq.cdb_list import get_lists, get_path_lists, iterate_lists, get_list_file, upload_list_file,\
            delete_list_file
        from cyb3rhq.core import common
        from cyb3rhq.core.results import AffectedItemsCyb3rhqResult

RELATIVE_PATH = os.path.join("framework", "cyb3rhq", "tests", "data", "test_cdb_list")
NAME_FILE_1 = "test_lists_1"
NAME_FILE_2 = "test_lists_2"
NAME_FILES = [NAME_FILE_1, NAME_FILE_2]

RESULT_GET_LIST_FILE_1 = [{'items': [{'key': 'test-cyb3rhq-w', 'value': 'write'},
                                     {'key': 'test-cyb3rhq-r', 'value': 'read'},
                                     {'key': 'test-cyb3rhq-a', 'value': 'attribute'},
                                     {'key': 'test-cyb3rhq-x', 'value': 'execute'},
                                     {'key': 'test-cyb3rhq-c', 'value': 'command'}
                                     ],
...
@patch('cyb3rhq.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
@pytest.mark.parametrize("search_text, complementary_search, search_in_fields, paths, expected_result", [
    ...
    ("test-cyb3rhq-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-ossec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-cyb3rhq-w", False, "items", [NAME_FILE_2], []),
    ...
    ("test-cyb3rhq-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-ossec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-cyb3rhq-w", True, "items", [NAME_FILE_2], RESULT_GET_LIST_FILE_2),
])
@patch('cyb3rhq.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsCyb3rhqResult)
    assert isinstance(result_b, AffectedItemsCyb3rhqResult)
...
@patch('cyb3rhq.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
@patch('cyb3rhq.cdb_list.iterate_lists', side_effect=lists_path_mock)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    """
    with patch('cyb3rhq.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        result = get_list_file([filename], raw)
        if raw:
            assert result == expected_result
        else:
            isinstance(result, AffectedItemsCyb3rhqResult)
...
@patch('cyb3rhq.cdb_list.safe_move')
@patch('cyb3rhq.cdb_list.delete_file_with_backup')
@patch('cyb3rhq.cdb_list.upload_file')
@patch('cyb3rhq.cdb_list.delete_list_file')
@patch('cyb3rhq.cdb_list.remove')
@patch('cyb3rhq.cdb_list.exists', return_value=True)
...
@patch('cyb3rhq.cdb_list.common.USER_LISTS_PATH', return_value='/test/path')
@patch('cyb3rhq.cdb_list.remove')
def test_upload_list_file_ko(mock_remove, mock_lists_path):
    ...
    with patch('cyb3rhq.cdb_list.safe_move') as mock_safe_move:
        with patch('cyb3rhq.cdb_list.exists', return_value=True):
            ...
            with patch('cyb3rhq.cdb_list.get_filenames_paths', return_value=['/test']):
...
def test_delete_list_file(mock_delete_file):
    ...
    try:
        ...
        with patch('cyb3rhq.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH):
            result = delete_list_file(['test_file'])
            assert result.render()['data']['affected_items'][0] ==\
                   'framework/cyb3rhq/tests/data/test_cdb_list/test_file'
    finally:
        try:
            os.remove(test_file)
        except Exception:
