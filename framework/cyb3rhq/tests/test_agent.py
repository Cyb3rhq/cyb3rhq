...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        ...
        from cyb3rhq import Cyb3rhqError, Cyb3rhqException, Cyb3rhqInternalError
        from cyb3rhq.agent import (
        ...
        from cyb3rhq.core.indexer.agent import Agent as IndexerAgent
        ...
        from cyb3rhq.core.tests.test_agent import InitAgent
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'Cyb3rhq v3.9.0', 'count': 1}, {'version': 'Cyb3rhq v3.8.2', 'count': 2},
             {'version': 'Cyb3rhq v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('cyb3rhq.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsCyb3rhqResult), 'The returned object is not an "AffectedItemsCyb3rhqResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsCyb3rhqResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, Cyb3rhqResult), 'The returned object is not an "Cyb3rhqResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsCyb3rhqResult), 'The returned object is not an "Cyb3rhqResult" instance.'
...
@patch('cyb3rhq.core.agent.Agent.reconnect')
@patch('cyb3rhq.agent.get_agents_info', return_value=short_agent_list)
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsCyb3rhqResult), 'The returned object is not an "AffectedItemsCyb3rhqResult".'
...
@patch('cyb3rhq.agent.send_restart_command')
@patch('cyb3rhq.agent.get_agents_info', return_value=set(short_agent_list))
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('cyb3rhq.agent.get_agents')
@patch('cyb3rhq.agent.get_groups')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~cyb3rhq,status~active)', 'group=default;((name~cyb3rhq,status~active))'),
    ('default', 'name~cyb3rhq,status~active', 'group=default;(name~cyb3rhq,status~active)')
])
@patch('cyb3rhq.agent.get_agents')
@patch('cyb3rhq.agent.get_groups')
...
@patch('cyb3rhq.agent.get_source_items_id')
@patch('cyb3rhq.core.indexer.create_indexer')
...
async def test_agent_add_agent_ko(create_indexer_mock, name, id, key):
    ...
    key : str
        The agent key.
    """
    with pytest.raises(Cyb3rhqError, match='.* 1738 .*'):
...
@patch('cyb3rhq.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('cyb3rhq.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('cyb3rhq.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('cyb3rhq.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, Cyb3rhqResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, Cyb3rhqError(1006)),
    (test_shared_path, ['default'], True, Cyb3rhqError(1405), Cyb3rhqError(1405)),
    (test_shared_path, ['default'], True, Cyb3rhqException(1400), Cyb3rhqInternalError(1727))
])
@patch('cyb3rhq.agent.process_array')
@patch('cyb3rhq.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('cyb3rhq.core.common.SHARED_PATH', new=shared_path):
        ...
        except (Cyb3rhqError, Cyb3rhqInternalError) as e:
...
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
@patch('cyb3rhq.core.common.cyb3rhq_gid', return_value=getgrnam('root'))
@patch('cyb3rhq.core.common.cyb3rhq_uid', return_value=getpwnam('root'))
@patch('cyb3rhq.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, Cyb3rhqResult), 'The returned object is not an "Cyb3rhqResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', Cyb3rhqError, 1711),
    ('group-1', Cyb3rhqError, 1711),
    ('invalid!', Cyb3rhqError, 1722),
    ('delete-me', Cyb3rhqInternalError, 1005),
    ('ar.conf', Cyb3rhqError, 1713),
    ('agent-template.conf', Cyb3rhqError, 1713)
])
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the Cyb3rhq Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('cyb3rhq.agent.get_groups')
@patch('cyb3rhq.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [Cyb3rhqResourceNotFound(1710)]),
    (['default'], [Cyb3rhqError(1712)]),
    (['none-1', 'none-2'], [Cyb3rhqResourceNotFound(1710)]),
    (['default', 'none-1'], [Cyb3rhqError(1712), Cyb3rhqResourceNotFound(1710)]),
])
@patch('cyb3rhq.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of Cyb3rhqError
        List of expected Cyb3rhqError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('cyb3rhq.agent.Agent.add_group_to_agent')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], Cyb3rhqResourceNotFound(1710), True),
    (['group-1'], ['100'], Cyb3rhqResourceNotFound(1701), False),
    (['default'], ['000'], Cyb3rhqError(1703), False)
])
@patch('cyb3rhq.agent.Agent.group_exists')
@patch('cyb3rhq.agent.Agent.add_group_to_agent')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsCyb3rhqResult` containing the exceptions in its 'failed_items'.
    """
...
@patch('cyb3rhq.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('cyb3rhq.core.agent.Agent.unset_single_group_agent')
@patch('cyb3rhq.agent.get_groups')
@patch('cyb3rhq.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', Cyb3rhqResourceNotFound(1701)),
    ('any-group', '000', Cyb3rhqError(1703)),
    ('group-1', '005', Cyb3rhqResourceNotFound(1710)),
])
@patch('cyb3rhq.agent.get_agents_info', return_value=short_agent_list)
@patch('cyb3rhq.agent.get_groups', side_effect={'default'})
def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : Cyb3rhqError
        The Cyb3rhqError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (Cyb3rhqError, Cyb3rhqResourceNotFound) as error:
...
@patch('cyb3rhq.core.agent.Agent.unset_single_group_agent')
@patch('cyb3rhq.agent.get_agents_info', return_value=short_agent_list)
@patch('cyb3rhq.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], Cyb3rhqResourceNotFound(1701), True),
    (['any-group'], ['000'], Cyb3rhqError(1703), True),
    (['any-group'], ['005'], Cyb3rhqResourceNotFound(1710), False),
])
@patch('cyb3rhq.core.agent.Agent.unset_single_group_agent')
@patch('cyb3rhq.agent.get_agents_info', return_value=short_agent_list)
@patch('cyb3rhq.agent.get_groups', return_value={'group-1'})
def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            'An "Cyb3rhqError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsCyb3rhqResult), 'The returned object is not an "AffectedItemsCyb3rhqResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            'No exception should be raised at this point. An AffectedItemsCyb3rhqResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], Cyb3rhqResourceNotFound(1710), True),
    (['group-1'], ['000'], Cyb3rhqError(1703), False),
    (['group-1'], ['100'], Cyb3rhqResourceNotFound(1701), False),
])
@patch('cyb3rhq.agent.get_agents_info', return_value=short_agent_list)
@patch('cyb3rhq.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating Cyb3rhqInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('cyb3rhq.agent.get_agents_info', return_value=set(full_agent_list))
@patch('cyb3rhq.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('cyb3rhq.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a Cyb3rhq Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(Cyb3rhqError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(Cyb3rhqInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('cyb3rhq.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a Cyb3rhq Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(Cyb3rhqInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, cyb3rhq_socket_mock, agent_list, component, configuration):
    ...
    """
    cyb3rhq_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    try:
        ...
        assert error == Cyb3rhqError(1740)
...
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
@patch('cyb3rhq.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('cyb3rhq.agent.get_agents_info', return_value=full_agent_list)
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['000'], Cyb3rhqError(1703)),
    (['100'], Cyb3rhqResourceNotFound(1701))
])
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : Cyb3rhqError
        Expected Cyb3rhqError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsCyb3rhqResult), 'The returned object is not an "AffectedItemsCyb3rhqResult".'
    # Check failed items
...
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
@patch('cyb3rhq.core.configuration.upload_group_configuration')
...
@patch('cyb3rhq.core.common.SHARED_PATH', new=test_shared_path)
@patch('cyb3rhq.agent.get_distinct_agents')
@patch('cyb3rhq.agent.get_agent_groups')
@patch('cyb3rhq.agent.get_agents_summary_status')
@patch('cyb3rhq.agent.get_agents')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('cyb3rhq.agent.get_agents_info', return_value=['test', 'test2'])
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('cyb3rhq.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('cyb3rhq.core.agent.Agent.get_agent_groups')
@patch('cyb3rhq.core.agent.Agent.set_agent_group_file')
@patch('cyb3rhq.core.agent.Agent')
...
@patch('cyb3rhq.core.agent.Agent.group_exists', return_value=False)
...
def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected Cyb3rhqException code error.
    """
    with pytest.raises(Cyb3rhqException, match=f".* {expected_exc} .*"):
