...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'cyb3rhq-remoted',
                               '/var/ossec/queue/sockets/analysis': 'cyb3rhq-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'cyb3rhq-remoted': '/var/ossec/queue/sockets/remote',
                               'cyb3rhq-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('cyb3rhq.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsCyb3rhqResult), 'The result is not Cyb3rhqResult type'
...
@pytest.mark.asyncio
@patch('cyb3rhq.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('cyb3rhq.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('cyb3rhq.core.common.WDB_SOCKET', '/var/ossec/queue/db/wdb')
@patch('cyb3rhq.stats.get_daemons_stats_socket')
async def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = await stats.get_daemons_stats(['cyb3rhq-remoted', 'cyb3rhq-analysisd', 'cyb3rhq-db'])
    ...
    assert isinstance(response, AffectedItemsCyb3rhqResult), \
            'The result is not AffectedItemsCyb3rhqResult type'
    assert response.total_affected_items == len(response.affected_items)


@pytest.mark.asyncio
@patch('cyb3rhq.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/wrong_socket_name')
async def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = await stats.get_daemons_stats(['cyb3rhq-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['cyb3rhq-remoted', 'cyb3rhq-analysisd']),
    (['cyb3rhq-remoted'], ['cyb3rhq-remoted']),
    (['cyb3rhq-remoted', 'cyb3rhq-analysisd'], ['cyb3rhq-remoted', 'cyb3rhq-analysisd'])
])
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('cyb3rhq.stats.get_agents_info', return_value={'000', '001', '002', '003', '004', '005'})
@patch('cyb3rhq.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('cyb3rhq.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('cyb3rhq.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
async def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, 
    ...
    assert isinstance(result, AffectedItemsCyb3rhqResult), \
        'The result is not an AffectedItemsCyb3rhqResult object'
...
@patch('cyb3rhq.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('cyb3rhq.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('cyb3rhq.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('cyb3rhq.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('cyb3rhq.core.agent.Agent.get_stats')
@patch('cyb3rhq.stats.get_agents_info', return_value=['000', '001'])
def test_get_agents_component_stats_json(mock_agents_info, mock_getstats, component):
    ...
    assert isinstance(response, AffectedItemsCyb3rhqResult), 'The result is not AffectedItemsCyb3rhqResult type'
