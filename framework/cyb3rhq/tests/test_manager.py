...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        ...
        from cyb3rhq.manager import *
        from cyb3rhq.core.manager import LoggingFormat
        from cyb3rhq.core.tests.test_manager import get_logs
        from cyb3rhq import Cyb3rhqInternalError
...
def mock_cyb3rhq_path():
    with patch('cyb3rhq.core.common.CYB3RHQ_PATH', new=test_data_path):
        yield
...
manager_status = {'cyb3rhq-agentlessd': 'running', 'cyb3rhq-analysisd': 'running', 'cyb3rhq-authd': 'running',
 'cyb3rhq-csyslogd': 'running', 'cyb3rhq-dbd': 'running', 'cyb3rhq-monitord': 'running',
 'cyb3rhq-execd': 'running', 'cyb3rhq-integratord': 'running', 'cyb3rhq-logcollector': 'running',
 'cyb3rhq-maild': 'running', 'cyb3rhq-remoted': 'running', 'cyb3rhq-reportd': 'running',
 'cyb3rhq-syscheckd': 'running', 'cyb3rhq-clusterd': 'running', 'cyb3rhq-modulesd': 'running',
 'cyb3rhq-db': 'running', 'cyb3rhq-apid': 'running', 'cyb3rhq-comms-apid': 'running'}


@patch('cyb3rhq.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works"""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsCyb3rhqResult), 'No expected result type'
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('cyb3rhq-modulesd:database', None, 2, None, None),
    ('cyb3rhq-modulesd:syscollector', None, 2, None, None),
    ('cyb3rhq-modulesd:syscollector', None, 2, None, None),
    ('cyb3rhq-modulesd:aws-s3', None, 5, None, None),
    ('cyb3rhq-execd', None, 1, None, None),
    ('cyb3rhq-csyslogd', None, 2, None, None),
...
@patch("cyb3rhq.core.manager.get_cyb3rhq_active_logging_format", return_value=LoggingFormat.plain)
@patch("cyb3rhq.core.manager.exists", return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. cyb3rhq-remoted).
    total_items : int
    ...
    """
    with patch('cyb3rhq.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'cyb3rhq-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'cyb3rhq-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'cyb3rhq-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'cyb3rhq-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'cyb3rhq-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'cyb3rhq-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'cyb3rhq-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0}
    }

    logs = get_logs().splitlines()
    with patch('cyb3rhq.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('cyb3rhq.core.cluster.utils.fcntl')
@patch('cyb3rhq.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with pytest.raises(Cyb3rhqInternalError, match='.* 1901 .*'):
        restart()

    # Socket error
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(Cyb3rhqInternalError, match='.* 1902 .*'):
                restart()

        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                with pytest.raises(Cyb3rhqInternalError, match='.* 1014 .*'):
                    restart()


@pytest.mark.parametrize('error_flag, error_msg', [
    (0, ""),
    (1, "2019/02/27 11:30:07 cyb3rhq-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: "
        "Unspecified key"),
    (1, "2019/02/27 11:30:24 cyb3rhq-authd: ERROR: (1230): Invalid element in the configuration: "
        "'use_source_i'.\n2019/02/27 11:30:24 cyb3rhq-authd: ERROR: (1202): Configuration error at "
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('cyb3rhq.core.manager.Cyb3rhqSocket') as sock:
        # Mock sock response
...
@pytest.mark.parametrize('exception', [
    Cyb3rhqInternalError(1013),
    Cyb3rhqError(1013)
])
@patch('cyb3rhq.manager.validate_ossec_conf')
def test_validation_ko(mock_validate, exception):
    mock_validate.side_effect = exception

    if isinstance(exception, Cyb3rhqInternalError):
        with pytest.raises(Cyb3rhqInternalError, match='.* 1013 .*'):
            validation()
    else:
        result = validation()
...
@patch('cyb3rhq.core.configuration.get_active_configuration')
...
@patch('cyb3rhq.manager.validate_ossec_conf', return_value={'status': 'OK'})
@patch('cyb3rhq.manager.write_ossec_conf')
@patch('cyb3rhq.manager.validate_cyb3rhq_xml')
@patch('cyb3rhq.manager.full_copy')
@patch('cyb3rhq.manager.exists', return_value=True)
@patch('cyb3rhq.manager.remove')
@patch('cyb3rhq.manager.safe_move')
