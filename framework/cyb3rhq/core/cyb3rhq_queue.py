...
from cyb3rhq.core.common import origin_module
...
def create_cyb3rhq_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the Cyb3rhqQueue socket.
    ...
    -------
    str
        Message that will be sent to the Cyb3rhqQueue socket.
    """
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < Cyb3rhqQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, Cyb3rhqQueue.MAX_MSG_SIZE)
        except Exception:
            raise Cyb3rhqInternalError(1010, self.path)
    ...
    def _send(self, msg: bytes) -> None:
        ...
        try:
            ...
            if sent == 0:
                raise Cyb3rhqInternalError(1011, self.path)
        except socket.error:
            raise Cyb3rhqInternalError(1011, self.path)
...
class Cyb3rhqQueue(BaseQueue):
    ...
    RESTART_AGENTS_JSON = json.dumps(create_cyb3rhq_socket_message(origin={'module': origin_module.get()},
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        msg_is_no_ar = msg in [Cyb3rhqQueue.HC_SK_RESTART, Cyb3rhqQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [Cyb3rhqQueue.RESTART_AGENTS, Cyb3rhqQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == Cyb3rhqQueue.AR_TYPE:
            socket_msg = create_cyb3rhq_queue_socket_msg(flag, str_agent_id, msg) if agent_id != '000' else msg
            # Return message
        ...
        # Restart agents
            ...
            # If msg is not a non active-response command and not a restart command, raises Cyb3rhqInternalError
            if not msg_is_no_ar and not msg_is_restart:
                raise Cyb3rhqInternalError(1012, msg)
            socket_msg = create_cyb3rhq_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == Cyb3rhqQueue.HC_SK_RESTART:
                ret_msg = "Restarting Syscheck on agent" if agent_id else "Restarting Syscheck on all agents"
            elif msg == Cyb3rhqQueue.HC_FORCE_RECONNECT:
                ret_msg = "Reconnecting agent" if agent_id else "Reconnecting all agents"
            else:  # msg == Cyb3rhqQueue.RESTART_AGENTS or msg == Cyb3rhqQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise Cyb3rhqError(1014, extra_message=f": Cyb3rhqQueue socket with path {self.path}")

        return ret_msg


class Cyb3rhqAnalysisdQueue(BaseQueue):
    """
    Cyb3rhqAnalysisdQueue Object.
    """
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        try:
            ...
            raise Cyb3rhqError(1014, extra_message=f': Cyb3rhqAnalysisdQueue socket with path {self.path}. {str(e)}')
