...
from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqException, Cyb3rhqInternalError, Cyb3rhqHAPHelperError
from cyb3rhq.core.results import Cyb3rhqResult
from cyb3rhq.core.utils import temporary_cache
from cyb3rhq.core.cyb3rhq_socket import create_cyb3rhq_socket_message
from cyb3rhq.core.wlogging import Cyb3rhqLogger
...
execq_lockfile = os.path.join(common.CYB3RHQ_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'cyb3rhq_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise Cyb3rhqError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise Cyb3rhqError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise Cyb3rhqHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise Cyb3rhqError(3006, extra_message=e.message)
    except Exception as e:
        raise Cyb3rhqError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise Cyb3rhqError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise Cyb3rhqInternalError(1913, extra_message=str(e))

    processes = ['cyb3rhq-agentlessd', 'cyb3rhq-analysisd', 'cyb3rhq-authd', 'cyb3rhq-csyslogd', 'cyb3rhq-dbd', 'cyb3rhq-monitord',
                 'cyb3rhq-execd', 'cyb3rhq-integratord', 'cyb3rhq-logcollector', 'cyb3rhq-maild', 'cyb3rhq-remoted',
                 'cyb3rhq-reportd', 'cyb3rhq-syscheckd', 'cyb3rhq-clusterd', 'cyb3rhq-modulesd', 'cyb3rhq-db', 'cyb3rhq-apid',
                 'cyb3rhq-comms-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.CYB3RHQ_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['cyb3rhq-clusterd'] == 'running' else "no"}
...
def manager_restart() -> Cyb3rhqResult:
    """Restart Cyb3rhq manager.

    Send JSON message with the 'restart-cyb3rhq' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    Cyb3rhqInternalError(1901)
        If the socket path doesn't exist.
    Cyb3rhqInternalError(1902)
        If there is a socket connection error.
    Cyb3rhqInternalError(1014)
    ...
    try:
        ...
        # json msg for restarting Cyb3rhq manager
        msg = json.dumps(create_cyb3rhq_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_CYB3RHQ_COMMAND,
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise Cyb3rhqInternalError(1902)
        else:
            raise Cyb3rhqInternalError(1901)

        try:
            conn.send(msg.encode())
            conn.close()
        except socket.error as e:
            raise Cyb3rhqInternalError(1014, extra_message=str(e))
    finally:
        fcntl.lockf(lock_file, fcntl.LOCK_UN)
        lock_file.close()
        read_config.cache_clear()

    return Cyb3rhqResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.CYB3RHQ_PATH, here, 'cluster.json')) as f:
...
class ClusterLogger(Cyb3rhqLogger):
    """
    Define the logger used by cyb3rhq-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from cyb3rhq-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'cyb3rhq-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `Cyb3rhqResult` instance in case the execution did not fail. Return an exception otherwise.
    """
