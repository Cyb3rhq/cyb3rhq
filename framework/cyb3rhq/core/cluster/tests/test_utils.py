    ...
    with patch('cyb3rhq.core.common.getpwnam'):
        with patch('cyb3rhq.core.common.cyb3rhq_uid'):
            with patch('cyb3rhq.core.common.cyb3rhq_gid'):
                ...
                from cyb3rhq.core.cluster import utils
                from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqException, Cyb3rhqInternalError, Cyb3rhqPermissionError, \
                    Cyb3rhqResourceNotFound, Cyb3rhqHAPHelperError
...
def test_read_cluster_config():
    ...
    with patch('cyb3rhq.core.cluster.utils.get_ossec_conf', side_effect=Cyb3rhqError(1001)):
        with pytest.raises(Cyb3rhqError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('cyb3rhq.core.configuration.load_cyb3rhq_xml', return_value=SystemExit):
        with pytest.raises(SystemExit) as pytest_wrapped_e:
            utils.read_cluster_config(from_import=True)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 0

    with patch('cyb3rhq.core.cluster.utils.get_ossec_conf', side_effect=KeyError(1)):
        with pytest.raises(Cyb3rhqError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('cyb3rhq.core.cluster.utils.get_ossec_conf', return_value={'cluster': default_cluster_config}):
        ...
        with pytest.raises(Cyb3rhqError, match='.* 3004 .*'):
            utils.read_cluster_config()
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: Cyb3rhqException, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('cyb3rhq.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        with patch('re.match', return_value='None'):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'

        # with patch('cyb3rhq.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('cyb3rhq.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_manager_status_ko(mock_stat, exc):
    ...
    with pytest.raises(Cyb3rhqInternalError, match='.* 1913 .*'):
...
def test_get_cluster_status():
    ...
    Cyb3rhqInternalError is raised."""
    status = utils.get_cluster_status()
    assert {'enabled': 'no', 'running': 'no'} == status

    with patch('cyb3rhq.core.cluster.utils.get_manager_status', side_effect=Cyb3rhqInternalError(1913)):
        status = utils.get_cluster_status()
        assert {'enabled': 'no', 'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('cyb3rhq.core.cluster.utils.open', side_effect=None):
            ...
            with patch('os.path.exists', return_value=True):
                ...
                with patch('socket.socket.connect', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        assert Cyb3rhqResult({'message': 'Restart request sent'}) == status
...
def test_get_cluster_items():
    ...
    with patch('os.path.abspath', side_effect=FileNotFoundError):
        with pytest.raises(Cyb3rhqException, match='.* 3005 .*'):
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by cyb3rhq-clusterd."""
...
@patch('cyb3rhq.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'cyb3rhq-clusterd_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI')
...
@patch('cyb3rhq.core.cluster.utils.read_cluster_config')
