    ...
    with patch('wazuh.common.getpwnam'):
        with patch('wazuh.common.cyb3rhq_uid'):
            with patch('wazuh.common.cyb3rhq_gid'):
                ...
                from cyb3rhq.core.cluster import control
                from cyb3rhq.core.cluster.local_client import LocalClient
                from cyb3rhq import Cyb3rhqInternalError, Cyb3rhqError
...
async def test_get_nodes():
    ...
    with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
    ...
    with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', side_effect=[Cyb3rhqClusterError(3020), 'error']):
        with pytest.raises(Cyb3rhqClusterError):
            await control.get_nodes(lc=local_client)
...
async def test_get_system_nodes():
    ...
    with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
        ...
        for expected in expected_result:
            with patch('cyb3rhq.core.cluster.control.get_nodes', return_value=expected):
                result = await control.get_system_nodes()
                assert result == [expected['items'][0]['name']]

        with patch('cyb3rhq.core.cluster.control.get_nodes', side_effect=Cyb3rhqInternalError(3012)):
            result = await control.get_system_nodes()
            assert result == Cyb3rhqError(3013)
...
async def test_get_node_ruleset_integrity():
    ...
    local_client = LocalClient()
    with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client) as execute_mock:
