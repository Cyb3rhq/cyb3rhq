...
with patch('wazuh.common.cyb3rhq_uid'):
    with patch('wazuh.common.cyb3rhq_gid'):
        from cyb3rhq.core.cluster.local_client import *
        from cyb3rhq.core.cluster.common import InBuffer
...
@patch("cyb3rhq.core.cluster.client.asyncio.get_running_loop")
...
async def test_localclient_start():
    ...
    with patch("uvloop.Loop.create_unix_connection", side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch("os.path.join", return_value="path/test"):
            with patch("cyb3rhq.core.cluster.client.asyncio.get_running_loop", return_value=mocked_loop):
                lc = LocalClient()
                await lc.start()
...
async def test_localclient_start_ko(mock_get_running_loop):
    ...
    with pytest.raises(Cyb3rhqInternalError, match=r'.* 3009 .*'):
        await LocalClient().start()

    with patch("cyb3rhq.core.cluster.local_client.os.path.join", side_effect=MemoryError):
        with pytest.raises(Cyb3rhqInternalError, match=r'.* 1119 .*'):
            await LocalClient().start()

    with patch("cyb3rhq.core.cluster.local_client.os.path.join", side_effect=FileNotFoundError):
        with pytest.raises(Cyb3rhqInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()

    with patch("cyb3rhq.core.cluster.local_client.os.path.join", side_effect=ConnectionRefusedError):
        with pytest.raises(Cyb3rhqInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()
...
async def test_wait_for_response():
    ...
    lc.protocol.send_request.side_effect = [b"None", exception.Cyb3rhqClusterError(3018)]

    with patch("asyncio.Event.wait", side_effect=asyncio.TimeoutError):
        with pytest.raises(Cyb3rhqInternalError, match=rf".* 3020 .*"):
...
async def test_localclient_execute():
    ...
    with patch("cyb3rhq.core.cluster.local_client.LocalClient.start"):
        with patch("cyb3rhq.core.cluster.local_client.LocalClient.send_api_request", return_value="Test"):
...
async def test_localclient_send_file():
    ...
    with patch("cyb3rhq.core.cluster.local_client.LocalClient.start"):
        with patch("cyb3rhq.core.cluster.local_client.LocalClient.send_api_request", return_value=b"cyb3rhq/test python"):
            lc = LocalClient()
            assert await lc.send_file(path="cyb3rhq/test", node_name="python") == b"cyb3rhq/test python"
