...
with patch('wazuh.common.cyb3rhq_uid'):
    with patch('wazuh.common.cyb3rhq_gid'):
        ...
        from cyb3rhq.core.cluster.dapi.dapi import DistributedAPI, APIRequestQueue, SendSyncRequestQueue
        from cyb3rhq.core.manager import get_manager_status
        from cyb3rhq.core.results import Cyb3rhqResult, AffectedItemsCyb3rhqResult
        from cyb3rhq import agent, cluster, ciscat, manager, Cyb3rhqError, Cyb3rhqInternalError
        from cyb3rhq.core.exception import Cyb3rhqClusterError
        from api.util import raise_if_exc
        from cyb3rhq.core.cluster import local_client
...
@pytest.mark.parametrize('kwargs', [
    {'f_kwargs': {'select': ['id']}, 'rbac_permissions': {'mode': 'black'}, 'nodes': ['worker1'],
     'basic_services': ('cyb3rhq-modulesd', 'cyb3rhq-db'), 'request_type': 'local_master'},
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name="cyb3rhq-api")
    ...
    logger_ = TestingLogger(logger_name="cyb3rhq")
...
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=Cyb3rhqResult({'result': 'local'})))
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.forward_request',
       new=AsyncMock(return_value=Cyb3rhqResult({'result': 'forward'})))
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
       new=AsyncMock(return_value=Cyb3rhqResult({'result': 'remote'})))
...
def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, cluster_enabled, f_kwargs):
    ...
    with patch('cyb3rhq.core.cluster.dapi.dapi.check_cluster_status', return_value=cluster_enabled):
        with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': node}):
...
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=Cyb3rhqResult({'result': 'local'})))
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=Cyb3rhqResult({'unknown': ['001', '002']})))
...
def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger("cyb3rhq")
    with patch("cyb3rhq.core.cluster.dapi.dapi.get_node_wrapper", side_effect=Cyb3rhqError(4000)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        get_error_result = dapi.get_error_info(Exception("testing"))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'Cyb3rhq Internal Error. See log for more detail'

    with patch("cyb3rhq.core.cluster.dapi.dapi.get_node_wrapper", side_effect=Cyb3rhqError(4001)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        with pytest.raises(Cyb3rhqError, match='.* 4001 .*'):
            dapi.get_error_info(Exception("testing"))

    with patch("cyb3rhq.core.cluster.dapi.dapi.get_node_wrapper", return_value=NodeWrapper()):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
...
def test_DistributedAPI_local_request_errors():
    ...
    with patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
               new=AsyncMock(side_effect=Cyb3rhqInternalError(1001))):
        dapi_kwargs = {'f': agent.get_agents_summary_status, 'logger': logger}
        ...
        try:
            raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
        except Cyb3rhqInternalError as e:
            assert e.code == 1001
    ...
    with patch('cyb3rhq.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.check_cyb3rhq_status'):
...
def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=Cyb3rhqError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=Cyb3rhqInternalError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = Cyb3rhqClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, "error") as logger_error_mock:
            # Test Cyb3rhqClusterError caught in execute_local_request and ProblemException raised
            dapi_kwargs = {'f': manager.status, 'logger': testing_logger}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=exc_code)

            # Test Cyb3rhqClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
            except Cyb3rhqClusterError as e:
...
@patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'worker'})
@patch('cyb3rhq.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('cyb3rhq.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('cyb3rhq.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('cyb3rhq.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('cyb3rhq.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=Cyb3rhqResult({'testing': ['001', '002']})))
@patch('cyb3rhq.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
def test_DistributedAPI_tmp_file(mock_cluster_status):
    ...
    with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('cyb3rhq.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsCyb3rhqResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                           'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs)

    open('/tmp/dapi_file.txt', 'a').close()
    with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
def test_DistributedAPI_tmp_file_cluster_error(mock_cluster_status):
    ...
    with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('cyb3rhq.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsCyb3rhqResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=Cyb3rhqClusterError(3022))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            ...
            open('/tmp/dapi_file.txt', 'a').close()
            with patch('cyb3rhq.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=Cyb3rhqClusterError(1000))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
                raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1000)


@patch('cyb3rhq.core.cluster.local_client.LocalClient.execute',
       new=AsyncMock(return_value='{"items": [{"name": "master"}], "totalItems": 1}'))
@patch('cyb3rhq.agent.Agent.get_agents_overview', return_value={'items': [{'id': '001', 'node_name': 'master'},
...
def test_DistributedAPI_get_solver_node(mock_cluster_status, mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsCyb3rhqResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('cyb3rhq.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
            ...
            expected = AffectedItemsCyb3rhqResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('cyb3rhq.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [
    ...
    cyb3rhq.core.manager.status
])
@patch('cyb3rhq.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_cyb3rhq_status(status_mock, api_request):
    """Test `check_cyb3rhq_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_cyb3rhq_status()
    assert data is None
...
@patch('cyb3rhq.core.cluster.cluster.get_node', return_value={'node': 'random_node'})
def test_DistributedAPI_check_cyb3rhq_status_exception(node_info_mock, status_value):
    """Test exceptions from `check_cyb3rhq_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('cyb3rhq.core.manager.get_manager_status',
               return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger)
        try:
            dapi.check_cyb3rhq_status()
...
@patch("cyb3rhq.core.cluster.common.import_module", return_value="os.path")
...
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return 'cyb3rhq*request_queue*test ' \
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        logger_mock.assert_called_once_with("Error in DAPI request. The destination node is "
                                            "not connected or does not exist: 'cyb3rhq'.")

        node = NodeMock()
        with patch.object(node, "send_request", side_effect=Cyb3rhqClusterError(3020, extra_message="test")):
            with patch.object(node, "send_string", return_value=b"noerror"):
                with patch("cyb3rhq.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    server.clients = {"cyb3rhq": node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, "send_string", Exception("break while true")):
                with patch("cyb3rhq.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    with patch("cyb3rhq.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception()):
                        apirequest.logger = logging.getLogger("apirequest")
                        with pytest.raises(Exception):
                            await apirequest.run()


@patch("cyb3rhq.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception())
@patch("asyncio.get_event_loop")
async def test_SendSyncRequestQueue_run(loop_mock, contexlib_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return "cyb3rhq*request_queue*test {\"daemon_name\": \"test\"}"
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        with patch.object(node, "send_request", Exception("break while true")):
            with patch("cyb3rhq.core.cluster.dapi.dapi.cyb3rhq_sendasync", side_effect=Exception("break while true")):
                server.clients = {"cyb3rhq": node}
                sendsync.logger = logging.getLogger("sendsync")
                with pytest.raises(Exception):
                    await sendsync.run()

            with patch("cyb3rhq.core.cluster.dapi.dapi.cyb3rhq_sendasync", side_effect="noerror"):
                with pytest.raises(Exception):
                    await sendsync.run()
