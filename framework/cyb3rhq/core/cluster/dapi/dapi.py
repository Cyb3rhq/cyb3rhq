...
import cyb3rhq.core.cluster.cluster
import cyb3rhq.core.cluster.utils
import cyb3rhq.core.manager
import cyb3rhq.core.results as wresults
from cyb3rhq import agent
from cyb3rhq.cluster import get_node_wrapper, get_nodes_info
from cyb3rhq.core import common, exception
from cyb3rhq.core.cluster import local_client, common as c_common
from cyb3rhq.core.cluster.cluster import check_cluster_status
from cyb3rhq.core.exception import Cyb3rhqException, Cyb3rhqClusterError, Cyb3rhqError
from cyb3rhq.core.cyb3rhq_socket import cyb3rhq_sendasync
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = cyb3rhq.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        self.node_info = cyb3rhq.core.cluster.cluster.get_node() if node is None else node.get_node()
        ...
        if not basic_services:
            self.basic_services = ('cyb3rhq-modulesd', 'cyb3rhq-analysisd', 'cyb3rhq-execd', 'cyb3rhq-db', 'cyb3rhq-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'cyb3rhq-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.Cyb3rhqException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or Cyb3rhqException
            Dictionary with API response or Cyb3rhqException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_cyb3rhq_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractCyb3rhqResult, exception.Cyb3rhqException)) \
                else wresults.Cyb3rhqResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.Cyb3rhqInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.Cyb3rhqInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.Cyb3rhqClusterError))
            return e
        except exception.Cyb3rhqError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.Cyb3rhqInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_cyb3rhq_status(self):
        """
        There are some services that are required for cyb3rhq to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Cyb3rhq must be started before using the API is the services are stopped.

        The basic services cyb3rhq needs to be running are: cyb3rhq-modulesd, cyb3rhq-remoted, cyb3rhq-analysisd, cyb3rhq-execd
        and cyb3rhq-db
        """
        if self.f == cyb3rhq.core.manager.status:
            return

        status = cyb3rhq.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.Cyb3rhqError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.Cyb3rhqInternalError(3021)
                except OperationalError as exc:
                    raise exception.Cyb3rhqInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.Cyb3rhqInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.Cyb3rhqInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.Cyb3rhqInternalError(900)
            ...
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913} and not isinstance(e,
                                                                                           exception.Cyb3rhqClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.Cyb3rhqJSONEncoder)
        except (exception.Cyb3rhqError, exception.Cyb3rhqResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.Cyb3rhqJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.Cyb3rhqInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.Cyb3rhqJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.Cyb3rhqException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.Cyb3rhqException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of Cyb3rhqInternalError
        if isinstance(e, exception.Cyb3rhqInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('CYB3RHQ_HOME', os.path.relpath(h.baseFilename, start=common.CYB3RHQ_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.CYB3RHQ_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_cyb3rhq_object)
        os.remove(os.path.join(common.CYB3RHQ_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.Cyb3rhqJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_cyb3rhq_object)

    async def forward_request(self) -> [wresults.AbstractCyb3rhqResult, exception.Cyb3rhqException]:
        ...
        Returns
        -------
        wresults.AbstractCyb3rhqResult or exception.Cyb3rhqException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractCyb3rhqResult, exception.Cyb3rhqException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a Cyb3rhqError exception
                if isinstance(result, Cyb3rhqClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = Cyb3rhqError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractCyb3rhqResult, exception.Cyb3rhqException)) \
                else wresults.Cyb3rhqResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsCyb3rhqResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractCyb3rhqResult):
        ...
        # It might be a Cyb3rhqError after reducing
        if isinstance(response, wresults.AffectedItemsCyb3rhqResult):
...
class Cyb3rhqRequestQueue:
    """Represents a queue of Cyb3rhq requests"""
...
class APIRequestQueue(Cyb3rhqRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('cyb3rhq').getChild('dapi')
        self.logger.addFilter(cyb3rhq.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_cyb3rhq_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.Cyb3rhqJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(Cyb3rhqRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('cyb3rhq').getChild('sendsync')
        self.logger.addFilter(cyb3rhq.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await cyb3rhq_sendasync(**request)
