...
import pytest
from cyb3rhq.agent import get_agents, reconnect_agents
from cyb3rhq.cluster import get_nodes_info
from cyb3rhq.core.results import AffectedItemsCyb3rhqResult

from framework.cyb3rhq.core.cluster.hap_helper.cyb3rhq import Cyb3rhqAgent, Cyb3rhqDAPI


class TestCyb3rhqAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert Cyb3rhqAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert Cyb3rhqAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.cyb3rhq.core.cluster.hap_helper.cyb3rhq.DistributedAPI', autospec=True)
class TestCyb3rhqDAPI:
    cyb3rhq_dapi = Cyb3rhqDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsCyb3rhqResult()
    ...
    @mock.patch('framework.cyb3rhq.core.cluster.hap_helper.cyb3rhq.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsCyb3rhqResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.cyb3rhq_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.cyb3rhq_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.cyb3rhq_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.cyb3rhq_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.cyb3rhq_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': self.cyb3rhq_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.cyb3rhq_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsCyb3rhqResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.cyb3rhq_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': limit or self.cyb3rhq_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.cyb3rhq_dapi.logger,
            request_type='local_master',
        )
