...
from cyb3rhq.core import cluster as metadata, common, exception, utils
from cyb3rhq.core.agent import Agent
from cyb3rhq.core.cluster import server, cluster, common as c_common
from cyb3rhq.core.cluster.dapi import dapi
from cyb3rhq.core.cluster.utils import context_tag, log_subprocess_execution
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.cyb3rhq_common.integrity_check
    ...
    def done_callback(self, future=None):
        ...
        # if not self.cyb3rhq_common.extra_valid_requested:
        #     self.cyb3rhq_common.sync_integrity_free[0] = True
...
class ReceiveExtraValidTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.cyb3rhq_common.sync_extra_valid
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.cyb3rhq_common.extra_valid_requested = False
        self.cyb3rhq_common.sync_integrity_free[0] = True
...
class ReceiveAgentInfoTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.cyb3rhq_common.sync_cyb3rhq_db_info
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.cyb3rhq_common.sync_agent_info_free = True
...
class SendEntireAgentGroupsTask(c_common.SendStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.cyb3rhq_common.send_entire_agent_groups_information


class MasterHandler(server.AbstractServerHandler, c_common.Cyb3rhqCommon):
    """
    ...
    async def execute(self, command: bytes, data: bytes, wait_for_complete: bool = False) -> Dict:
        ...
        if command == b'dapi_fwd':
            ...
            if client in self.server.clients:
                ...
                raise exception.Cyb3rhqClusterError(3022, extra_message=client)
        ...
        if command == b'dapi' or command == b'dapi_fwd':
            try:
                ...
                raise exception.Cyb3rhqClusterError(3021)
        # Otherwise, immediately return the result obtained before.
        else:
            status, request_result = result
            if status != b'ok':
                raise exception.Cyb3rhqClusterError(3022, extra_message=request_result.decode())
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            Node name, cluster name, node type and cyb3rhq version all separated by spaces.
        ...
        if self.cluster_name != self.server.configuration['name']:
            raise exception.Cyb3rhqClusterError(3030)
        elif self.version != metadata.__version__:
            raise exception.Cyb3rhqClusterError(3031)

        # Create directory where zips and other files coming from or going to the worker will be managed.
        worker_dir = os.path.join(common.CYB3RHQ_PATH, 'queue', 'cluster', self.name)
        ...
        # SyncCyb3rhqdb instance to send agent-groups data to the worker.
        wdb_conn = AsyncCyb3rhqDBConnection()
        self.agent_groups = c_common.SyncCyb3rhqdb(manager=self, logger=self.task_loggers['Agent-groups send'],
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class Cyb3rhqCommon.
    ...
    def process_dapi_res(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if req_id in self.server.pending_api_requests:
            ...
            raise exception.Cyb3rhqClusterError(3032, extra_message=req_id)
    ...
    async def sync_cyb3rhq_db_info(self, task_id: bytes, info_type: str):
        """Create a process to send to the local cyb3rhq-db the chunks of data received from a worker.
    ...
    async def send_entire_agent_groups_information(self):
        ...
        sync_object = c_common.SyncCyb3rhqdb(manager=self, logger=logger, cmd=b'syn_g_m_w_c',
                                           data_retriever=AsyncCyb3rhqDBConnection().run_wdb_command,
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            ...
            raise exception.Cyb3rhqClusterError(3038, extra_message=str(e))
        finally:
            shutil.rmtree(decompressed_files_path)
                ...
                for file_path, data in files_metadata.items():
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = os.path.join(common.CYB3RHQ_PATH, unmerged_file_path)
                                # Path where to create the file before moving it to the destination path.
                                tmp_unmerged_path = os.path.join(common.CYB3RHQ_PATH, 'queue', 'cluster', worker_name,
                                ...
                                utils.safe_move(tmp_unmerged_path, full_unmerged_name,
                                                ownership=(common.cyb3rhq_uid(), common.cyb3rhq_gid()),
                    ...
                    # If the file is not 'merged' type, move it directly to the destination path.
                        ...
                        try:
                            ...
                            utils.safe_move(zip_path, full_path, ownership=(common.cyb3rhq_uid(), common.cyb3rhq_gid()),
                                            permissions=cluster_items['files'][item_key]['permissions'])
                        except TimeoutError as e:
                            raise e
                        except Exception as e:
...
class Master(server.AbstractServer):
    ...
    def __init__(self, **kwargs):
        ...
        # Handle exception when the user running Cyb3rhq cannot access /dev/shm
            ...
            self.logger.warning(
                "In order to take advantage of Cyb3rhq 4.3.0 cluster improvements, the directory '/dev/shm' must be "
                "accessible by the 'cyb3rhq' user. Check that this file has permissions to be accessed by all users. "
                "Changing the file permissions to 777 will solve this issue.")
            self.logger.warning(
                "The Cyb3rhq cluster will be run without the improvements added in Cyb3rhq 4.3.0 and higher versions.")
            self.task_pool = None
    ...
    async def agent_groups_update(self):
        ...
        wdb_conn = AsyncCyb3rhqDBConnection()
        sync_object = c_common.SyncCyb3rhqdb(manager=self, logger=logger, cmd=b'syn_g_m_w',
