...
from cyb3rhq.core.indexer import Indexer, create_indexer, get_indexer_client
...
class TestIndexer:
    ...
    def test_indexer_init_ko(self, params: dict):
        """Check the correct initalization of the `Indexer` class."""
        with pytest.raises(Cyb3rhqIndexerError, match='.*2201.*'):
    ...
    async def test_connect_ko(self, indexer_instance_with_mocked_client):
        ...
        with pytest.raises(Cyb3rhqIndexerError, match='.*2200.*'):
            await indexer_instance_with_mocked_client.connect()
...
@mock.patch('cyb3rhq.core.indexer.Indexer', autospec=True)
...
async def test_create_indexer_ko(indexer_mock: mock.AsyncMock, retries: int):
    ...
    instance_mock.connect.side_effect = Cyb3rhqIndexerError(2200)
    indexer_mock.return_value = instance_mock

    with mock.patch('cyb3rhq.core.indexer.sleep') as sleep_mock:
        with pytest.raises(Cyb3rhqIndexerError, match='.*2200.*'):
...
@mock.patch('cyb3rhq.core.indexer.create_indexer')
