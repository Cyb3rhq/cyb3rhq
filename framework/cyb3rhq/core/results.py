...
import cyb3rhq.core.exception as wexception
...
class AbstractCyb3rhqResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.Cyb3rhqInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractCyb3rhqResult):
            self.dikt = dct.dikt
        else:
            raise wexception.Cyb3rhqInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractCyb3rhqResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractCyb3rhqResult objects. When merged with a Cyb3rhqException, the result is
        always a Cyb3rhqException

        Parameters
        ----------
        other : AbstractCyb3rhqResult or wexception.Cyb3rhqException

        Raises
        ------
        Cyb3rhqInternalError(1000)
            Cyb3rhq internal error when two object could not be merged.

        Returns
        -------
        AbstractCyb3rhqResult or wexception.Cyb3rhqException
            Resultant object.
        """
        if isinstance(other, wexception.Cyb3rhqException):
            return other
        elif not isinstance(other, (dict, AbstractCyb3rhqResult)):
            raise wexception.Cyb3rhqInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractCyb3rhqResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in Cyb3rhqResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in cyb3rhq.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractCyb3rhqResult
            Filtered AbstractCyb3rhqResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in Cyb3rhqResult subclasses.
...
class Cyb3rhqResult(AbstractCyb3rhqResult):
    """
...
class AffectedItemsCyb3rhqResult(AbstractCyb3rhqResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.Cyb3rhqException = None):
        ...
        error : wexception.Cyb3rhqException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsCyb3rhqResult
        ...
        wexception.Cyb3rhqInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsCyb3rhqResult):
            raise wexception.Cyb3rhqInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsCyb3rhqResult with self.

        Parameters
        ----------
        other : AffectedItemsCyb3rhqResult
            AffectedItemsCyb3rhqResult object to merge.
        ...
        if isinstance(other, wexception.Cyb3rhqError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.Cyb3rhqException):
            return result
        elif not isinstance(other, AffectedItemsCyb3rhqResult):
            raise wexception.Cyb3rhqInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsCyb3rhqResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractCyb3rhqResult
            Instance of AbstractCyb3rhqResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsCyb3rhqResult object.

        Returns
        -------
        dict
            Return AffectedItemsCyb3rhqResult as a dictionary with fields data, message and error.
        """
