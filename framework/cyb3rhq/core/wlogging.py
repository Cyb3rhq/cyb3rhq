...
class TimeBasedFileRotatingHandler(logging.handlers.TimedRotatingFileHandler):
    """
    Cyb3rhq log rotation. It rotates the log at midnight and sets the appropriate permissions to the new log file.
    """
    ...
    def doRollover(self):
        ...
        # Save rotated file in {CYB3RHQ_PATH}/logs/api directory
...
class SizeBasedFileRotatingHandler(logging.handlers.RotatingFileHandler):
    """Cyb3rhq log rotation. It rotates when the logging file size exceeds the maximum number of bytes configured."""
...
class Cyb3rhqLogger:
    """
    Define attributes of a Python Cyb3rhq daemon's logger.
    """
    def __init__(self, foreground_mode: bool, log_path: str, debug_level: [int, str], logger_name: str = 'cyb3rhq',
        ...
        log_path : str
            Filepath of the file to send logs to. Relative to the Cyb3rhq installation path.
        ...
        """
        self.log_path = os.path.join(common.CYB3RHQ_PATH, log_path)
        self.logger = None
...
class CLIFilter(logging.Filter):
    """
    Define a custom filter to filter Cyb3rhqInternalErrors
    """

    messages_to_avoid = ['Cyb3rhq Internal Error', 'Cyb3rhqInternalError']
