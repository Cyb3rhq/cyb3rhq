...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        ...
        from cyb3rhq.core import common, stats
        from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqException, Cyb3rhqInternalError
...
def test_totals_ko_():
    ...
    with patch('cyb3rhq.core.stats.open', side_effect=IOError):
        with pytest.raises(Cyb3rhqException, match=".* 1308 .*"):
            stats.totals_(date(1996, 8, 13))

    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(Cyb3rhqInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))


@patch('cyb3rhq.core.common.STATS_PATH', new=test_data_path)
def test_weekly_():
...
@patch('cyb3rhq.core.common.STATS_PATH', new='')
...
@pytest.mark.parametrize('agent, daemon, expected_value', [
    (0, 'logcollector', os.path.join(common.CYB3RHQ_PATH, "queue", "sockets", 'logcollector')),
...
@patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqAsyncSocketJSON.close')
@patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqAsyncSocketJSON.send')
@patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqAsyncSocketJSON.connect')
async def test_get_daemons_stats_socket(mock_connect, mock_send, mock_close, 
    ...
    with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqAsyncSocketJSON.receive',
               return_value=expected_socket_response) as mock_receive, \
         patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqAsyncSocketJSON.receive_json',
...
async def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(Cyb3rhqInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_ko():
    ...
    with patch('cyb3rhq.core.stats.open'):
        with pytest.raises(Cyb3rhqInternalError, match=".* 1104 .*"):
            stats.get_daemons_stats_('filename')
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.__init__', return_value=None) as mock_socket:
        with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.send', side_effect=None) as mock_send:
            with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.receive', return_value=response.encode()):
                with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        if agent_id == '000':
            mock_socket.assert_called_once_with(os.path.join(common.CYB3RHQ_PATH, "queue", "sockets", "logcollector"))
            mock_send.assert_called_once_with(b'getstate')
        else:
            mock_socket.assert_called_once_with(os.path.join(common.CYB3RHQ_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.__init__', return_value=None) as mock_socket:
        with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.send', side_effect=None) as mock_send:
            with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.receive', side_effect=responses):
                with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.close', side_effect=None):
...
def test_get_daemons_stats_from_socket_ko():
    ...
    with pytest.raises(Cyb3rhqError, match=r'\b1307\b'):
        stats.get_daemons_stats_from_socket(None, None)

    with pytest.raises(Cyb3rhqError, match=r'\b1310\b'):
        stats.get_daemons_stats_from_socket('000', 'agent')

    with pytest.raises(Cyb3rhqInternalError, match=r'\b1121\b'):
        stats.get_daemons_stats_from_socket('000', 'logcollector')

    with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.__init__', return_value=None):
        with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.close', side_effect=None):
            with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.send', side_effect=None):
                with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.receive', side_effect=ValueError):
                    with pytest.raises(Cyb3rhqInternalError, match=r'\b1118\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')

                with patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket.receive',
                           return_value=json.dumps({'error': 1}).encode()):
                    with pytest.raises(Cyb3rhqError, match=r'\b1117\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')
