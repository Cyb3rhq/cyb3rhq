...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core.agent import *
        from cyb3rhq.core.exception import Cyb3rhqException
        from api.util import remove_nones_to_dict
        from cyb3rhq.core.common import reset_context_cache
...
def get_Cyb3rhqDBQuery_params(wdb_class):
    """Get default parameters for the specified Cyb3rhqDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the Cyb3rhqDBQuery class. Example: 'Agents' to get default parameters from `Cyb3rhqDBQueryAgents` class.
    ...
    """
    with patch('cyb3rhq.core.agent.Cyb3rhqDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'Cyb3rhqDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with Cyb3rhq packages availables with their hash
...
@patch("cyb3rhq.core.agent.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of Cyb3rhqDBQueryAgents works properly.
    ...
    if value:
        Cyb3rhqDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(Cyb3rhqException, match=".* 2005 .*"):
            Cyb3rhqDBQueryAgents()


@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of Cyb3rhqDBQueryAgents returns expected query"""
    query_agent = Cyb3rhqDBQueryAgents()
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of Cyb3rhqDBQueryAgents returns expected result
    ...
    """
    query_agent = Cyb3rhqDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of Cyb3rhqDBQueryAgents returns expected query"""
    query_agent = Cyb3rhqDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of Cyb3rhqDBQueryAgents returns expected data"""
    query_agent = Cyb3rhqDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of Cyb3rhqDBQueryAgents returns expected query"""
    query_agent = Cyb3rhqDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of Cyb3rhqDBQueryAgents returns expected query
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of Cyb3rhqDBQueryGroup works properly.
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of Cyb3rhqDBQueryGroup works properly.
        ...
        """
    query_group = Cyb3rhqDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of Cyb3rhqDBQueryGroupByAgents works properly."""
    query_group = Cyb3rhqDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of Cyb3rhqDBQueryGroupByAgents works properly."""
    query_group = Cyb3rhqDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'cyb3rhq-master'},
                         {'count': 1, 'name': 'cyb3rhq-agent1'}]
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if Cyb3rhqDBQueryGroupByAgents works properly."""
    query_group = Cyb3rhqDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of Cyb3rhqDBQueryGroup works properly"""
    query_group = Cyb3rhqDBQueryGroup()
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of Cyb3rhqDBQueryMultigroups works properly."""
    query_multigroups = Cyb3rhqDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of Cyb3rhqDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = Cyb3rhqDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of Cyb3rhqDBQueryMultigroups works properly."""
    query_multigroups = Cyb3rhqDBQueryMultigroups(group_id='test')
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Cyb3rhqDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of Cyb3rhqDBQueryMultigroups works properly."""
    query_multigroups = Cyb3rhqDBQueryMultigroups(group_id='test')
...
@patch('cyb3rhq.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(Cyb3rhqResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(Cyb3rhqError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()


@patch('cyb3rhq.core.agent.Cyb3rhqQueue.send_msg_to_agent')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(Cyb3rhqQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(Cyb3rhqQueue.HC_FORCE_RECONNECT, agent_id)


@patch('cyb3rhq.core.agent.Cyb3rhqQueue')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(Cyb3rhqError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('cyb3rhq.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('cyb3rhq.core.agent.get_manager_status', return_value={'cyb3rhq-authd': 'running'}):
        agent = Agent('000')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(Cyb3rhqError, match='.* 1726 .*'):
        agent = Agent('000')
        agent.remove()


@patch('cyb3rhq.core.agent.Cyb3rhqSocketJSON')
def test_agent_remove_authd(mock_cyb3rhq_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('000')
    agent._remove_authd(purge=True)
    mock_cyb3rhq_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(0).zfill(3), "purge": True}})
    mock_cyb3rhq_socket.return_value.receive.assert_called_once()
    mock_cyb3rhq_socket.return_value.close.assert_called_once()
...
@patch('cyb3rhq.core.agent.Agent._add_authd')
...
@patch('cyb3rhq.core.agent.get_manager_status', return_value={'cyb3rhq-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(Cyb3rhqError, match='.* 1706 .*'):
...
@patch('cyb3rhq.core.agent.Cyb3rhqSocketJSON')
def test_agent_add_authd(mock_cyb3rhq_socket, name, ip, id, key, force):
    ...
    mock_cyb3rhq_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (Cyb3rhqError(9008, cmd_error=True), ".* 1705 .*"),
    (Cyb3rhqError(9007, cmd_error=True), ".* 1706 .*"),
    (Cyb3rhqError(9012, cmd_error=True), ".* 1708 .*"),
    (Cyb3rhqError(9000, cmd_error=True), ".* None")
])
@patch('cyb3rhq.core.agent.Cyb3rhqSocketJSON')
def test_agent_add_authd_ko(mock_cyb3rhq_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(Cyb3rhqError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_cyb3rhq_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(Cyb3rhqError, match=expected_exception):
...
@patch('cyb3rhq.core.agent.rmtree')
@patch('cyb3rhq.core.agent.path.exists', return_value=True)
@patch('cyb3rhq.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('cyb3rhq.core.agent.Agent.get_agent_groups', return_value=['default'])
@patch('cyb3rhq.core.agent.Agent.set_agent_group_relationship')
...
def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('cyb3rhq.core.agent.Agent.get_agent_groups', side_effect=Cyb3rhqError(2003)):
        with pytest.raises(Cyb3rhqInternalError, match='.* 2007 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(Cyb3rhqError, match='.* 1722 .*'):
        Agent.group_exists('default**')


@patch('cyb3rhq.core.agent.Cyb3rhqDBConnection.send', return_value=('ok', '["payload"]'))
@patch('socket.socket.connect')
...
@patch('cyb3rhq.core.agent.Cyb3rhqDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(Cyb3rhqInternalError, match='.* 2005 .*'):
...
@patch('cyb3rhq.core.agent.Agent.set_agent_group_relationship')
@patch('cyb3rhq.core.agent.Agent.group_exists', return_value=True)
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('cyb3rhq.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('cyb3rhq.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(Cyb3rhqResourceNotFound, match='.* 1710 .*'):
            Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('cyb3rhq.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(Cyb3rhqError, match='.* 1734 .*'):
            Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('cyb3rhq.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(Cyb3rhqError, match='.* 1745 .*'):
            Agent.unset_single_group_agent('002', 'default', force=True)


@patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_cyb3rhq_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_cyb3rhq_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'Cyb3rhq v4.0.0')
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_cyb3rhq_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(Cyb3rhqError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Cyb3rhq v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(Cyb3rhqError, match=".* 1307 .*"):
        agent.get_config('com', None, 'Cyb3rhq v4.0.0')
        agent.get_config(None, 'active-response', 'Cyb3rhq v4.0.0')

    # Agent Cyb3rhq version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(Cyb3rhqInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'Cyb3rhq v3.6.0')


@patch('cyb3rhq.core.cyb3rhq_socket.Cyb3rhqSocket')
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_cyb3rhq_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_cyb3rhq_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_cyb3rhq_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(Cyb3rhqInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Cyb3rhq v4.2.0', 'Cyb3rhq v4.0.0', 'Cyb3rhq v4.2.1', 'Cyb3rhq v3.13.2']])
])
@patch('cyb3rhq.core.agent.Cyb3rhqQueue.send_msg_to_agent')
@patch('cyb3rhq.core.agent.Cyb3rhqQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('cyb3rhq.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = Cyb3rhqQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = Cyb3rhqQueue.RESTART_AGENTS_JSON if Cyb3rhqVersion(
                agent_version['version']) >= Cyb3rhqVersion(common.AR_LEGACY_VERSION) else Cyb3rhqQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('cyb3rhq.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('cyb3rhq.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `Cyb3rhqDBConnection.send` method.
    expected_agents : set
    ...
    with patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection.send', side_effect=wdb_response):
...
@patch('cyb3rhq.core.agent.core_upgrade_agents')
