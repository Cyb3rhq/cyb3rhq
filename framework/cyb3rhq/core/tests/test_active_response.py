...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        ...
        from cyb3rhq.core import active_response
...
@pytest.mark.parametrize('agent_version, builder_type', [
    ('Cyb3rhq v4.2.0', active_response.ARJsonMessage),
    ('Cyb3rhq v4.3.0', active_response.ARJsonMessage),
    ('Cyb3rhq v4.1.0', active_response.ARStrMessage)
])
...
@pytest.mark.parametrize('expected_exception, command, arguments', [
    ...
    (1652, 'random', []),
    (1652, 'invalid_cmd', []),
    (None, 'restart-cyb3rhq0', []),
    (None, '!restart-cyb3rhq0', []),
    (None, 'restart-cyb3rhq0', ["arg1", "arg2"])
])
...
def test_create_message(expected_exception, command, arguments):
    ...
    if expected_exception:
        with pytest.raises(Cyb3rhqError, match=f'.* {expected_exception} .*'):
...
@pytest.mark.parametrize('expected_exception, command, arguments, alert', [
    (1650, None, [], None),
    (None, 'restart-cyb3rhq0', [], None),
    (None, 'restart-cyb3rhq0', [], None),
    (None, 'restart-cyb3rhq0', ["arg1", "arg2"], None),
...
def test_send_ar_message(agent_id, command, arguments, alert):
    ...
    with patch.object(Agent, 'get_basic_information', return_value=mock_agent_info) as mock_get_basic_info, \
        ...
        mock_wq.send_msg_to_agent.assert_called_once_with(msg=mock_message_builder_instance.create_message.return_value,
                                                          agent_id=agent_id,
                                                          msg_type=active_response.Cyb3rhqQueue.AR_TYPE)
