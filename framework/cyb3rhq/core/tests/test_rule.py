...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', Cyb3rhqError(1201))
])
@patch("cyb3rhq.core.common.CYB3RHQ_PATH", new=parent_directory)
@patch("cyb3rhq.core.common.RULES_PATH", new=data_path)
...
@patch("cyb3rhq.core.rule.load_cyb3rhq_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(Cyb3rhqError, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("cyb3rhq.core.rule.load_cyb3rhq_xml", side_effect=OSError(8, 'Error', 'Unknown'))
