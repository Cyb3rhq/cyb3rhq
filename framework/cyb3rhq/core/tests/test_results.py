...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core.results import Cyb3rhqResult, AffectedItemsCyb3rhqResult, _goes_before_than, nested_itemgetter, merge
        from cyb3rhq import Cyb3rhqException, Cyb3rhqError

param_name = ['affected_items', 'total_affected_items', 'sort_fields', 'sort_casting', 'sort_ascending',
              'all_msg', 'some_msg', 'none_msg']
CYB3RHQ_EXCEPTION_CODE = 1725
FAILED_AGENT_ID = '999'


@pytest.fixture(scope='function')
def get_cyb3rhq_result():
    return Cyb3rhqResult(dct={"data": {"items": [{"item1": "data1"}, {"item2": "OK"}], "message": "Everything ok"}},
                       str_priority=['KO', 'OK'])


@pytest.fixture(scope='function')
def get_cyb3rhq_affected_item():
    def _get_affected(params=None):
        kwargs = {p_name: param for p_name, param in zip(param_name, params)}
        return AffectedItemsCyb3rhqResult(**kwargs)

    return _get_affected


@pytest.fixture(scope='function')
def get_cyb3rhq_failed_item():
    item = AffectedItemsCyb3rhqResult()
    item.add_failed_item(id_=FAILED_AGENT_ID, error=Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE))
    return item
...
def test_results_Cyb3rhqResult__merge_str(dikt, priority, get_cyb3rhq_affected_item):
    """Test method `_merge_str` from `Cyb3rhqResult`.
        ...
        priority : list
            Used to set the Cyb3rhqResult priority.
        """
    cyb3rhq_result = Cyb3rhqResult(deepcopy(dikt), str_priority=priority)
    assert isinstance(cyb3rhq_result, Cyb3rhqResult)
    item2 = cyb3rhq_result.dikt['data']['items'][1]['item2']
    merge_result = cyb3rhq_result._merge_str(item2, 'KO')
    assert merge_result == priority[0] if priority else '{}|{}'.format(item2, 'KO')


def test_results_Cyb3rhqResult_to_dict(get_cyb3rhq_result):
    """Test method `to_dict` from `Cyb3rhqResult`."""
    dict_result = get_cyb3rhq_result.to_dict()
    assert isinstance(dict_result, dict)
    assert (key == result_key for key, result_key in zip(['str_priority', 'result'], dict_result.keys()))


def test_results_Cyb3rhqResult_render(get_cyb3rhq_result):
    """Test method `render` from `Cyb3rhqResult`."""
    render_result = get_cyb3rhq_result.render()
    assert isinstance(render_result, dict)
    assert render_result == get_cyb3rhq_result.dikt


def test_results_Cyb3rhqResult_decode_json(get_cyb3rhq_result):
    """Test class method `decode_json` from `Cyb3rhqResult`."""
    cyb3rhq_result = get_cyb3rhq_result
    decoded_result = Cyb3rhqResult.decode_json(cyb3rhq_result.to_dict())
    assert decoded_result == cyb3rhq_result
...
def test_results_AffectedItemsCyb3rhqResult(get_cyb3rhq_affected_item, param_value):
    """Test class `AffectedItemsCyb3rhqResult`.
    ...
    """
    affected_result = get_cyb3rhq_affected_item(param_value)
    assert isinstance(affected_result, AffectedItemsCyb3rhqResult)
    for value, dikt_value in zip(param_value, affected_result.dikt.values()):
        assert value == dikt_value


def test_results_AffectedItemsCyb3rhqResult_add_failed_item():
    """Test method `add_failed_item` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
    id_list = ['001', '002']
    # Add two failed items with different id but same exception
    for agent_id in id_list:
        affected_result.add_failed_item(id_=agent_id, error=Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE))

    assert affected_result.failed_items
    assert set(id_list) == next(iter(affected_result.failed_items.values()))


def test_results_AffectedItemsCyb3rhqResult_add_failed_items_from(get_cyb3rhq_failed_item):
    """Test method `add_failed_items_from` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
    failed_result = get_cyb3rhq_failed_item
    affected_result.add_failed_items_from(failed_result)
    assert affected_result.failed_items == failed_result.failed_items


def test_results_AffectedItemsCyb3rhqResult_add_failed_items_from_exception():
    """Test raised exception from method `add_failed_items_from` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
    with pytest.raises(Cyb3rhqException, match='.* 1000 .*'):
        affected_result.add_failed_items_from('Invalid type')


def test_results_AffectedItemsCyb3rhqResult_remove_failed_items(get_cyb3rhq_failed_item):
    """Test method `remove_failed_items` from class `AffectedItemsCyb3rhqResult`."""
    failed_result = get_cyb3rhq_failed_item
    failed_result.remove_failed_items(code={CYB3RHQ_EXCEPTION_CODE})
    assert not failed_result.failed_items


def test_results_AffectedItemsCyb3rhqResult___or__(get_cyb3rhq_failed_item):
    """Test method `__or__` from class `AffectedItemsCyb3rhqResult`."""
    agent_list_1 = ['001', '002']
    agent_list_2 = ['004', '003']
    affected_item_1 = AffectedItemsCyb3rhqResult(affected_items=deepcopy(agent_list_1))
    affected_item_2 = AffectedItemsCyb3rhqResult(affected_items=deepcopy(agent_list_2))
    failed_item = get_cyb3rhq_failed_item
...
@pytest.mark.parametrize('or_item, expected_result', [
    (Cyb3rhqError(CYB3RHQ_EXCEPTION_CODE, ids=['001']), AffectedItemsCyb3rhqResult),
    (Cyb3rhqError(CYB3RHQ_EXCEPTION_CODE), Cyb3rhqException),
    (Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE), Cyb3rhqException),
    ({'Invalid type': None}, None)
])
def test_results_AffectedItemsCyb3rhqResult___or___exceptions(or_item, expected_result):
    """Test raised exceptions from method `__or__` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
...
def test_results_AffectedItemsCyb3rhqResult_to_dict():
    """Test method `to_dict` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
...
def test_results_AffectedItemsCyb3rhqResult_properties():
    """Test getters and setters from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
...
def test_results_AffectedItemsCyb3rhqResult_failed_items_properties(get_cyb3rhq_failed_item):
    """Test `failed_items` properties from class `AffectedItemsCyb3rhqResult`."""
    fail_items = get_cyb3rhq_failed_item.failed_items
    total_fail_items = get_cyb3rhq_failed_item.total_failed_items
    assert isinstance(fail_items, dict)
    assert total_fail_items == 1


def test_results_AffectedItemsCyb3rhqResult_message_property():
    """Test `message` property from class `AffectedItemsCyb3rhqResult`."""
    ...
    none_msg_result = AffectedItemsCyb3rhqResult(**messages).message
    all_msg_result = AffectedItemsCyb3rhqResult(**messages, affected_items=['001']).message
    aux_result = AffectedItemsCyb3rhqResult(**messages, affected_items=['001'])
    aux_result.add_failed_item(Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE))
...
def test_results_AffectedItemsCyb3rhqResult__merge_str(self_field, other_field, key, expected_result):
    """Test method `_merge_str` from class `AffectedItemsCyb3rhqResult`."""
    affected_result = AffectedItemsCyb3rhqResult()
    merge_result = affected_result._merge_str(self_field, other_field, key=key)
    assert merge_result == expected_result


def test_results_AffectedItemsCyb3rhqResult_encode_decode_json(get_cyb3rhq_affected_item):
    """Test methods `encode_json` and `decode_json` from class `AffectedItemsCyb3rhqResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_cyb3rhq_affected_item(param_list)
    affected_result.add_failed_item(id_=FAILED_AGENT_ID, error=Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE))
    # Use a complete AffectedIemsCyb3rhqResult to encode a json and then decode it
    json_item = affected_result.encode_json()
    decoded_json = AffectedItemsCyb3rhqResult.decode_json(json_item)
    assert affected_result == decoded_json


def test_results_AffectedItemsCyb3rhqResult_render(get_cyb3rhq_affected_item):
    """Test method `render` from class `AffectedItemsCyb3rhqResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_cyb3rhq_affected_item(param_list)
    for agent_id in [FAILED_AGENT_ID, 'Invalid ID']:
        affected_result.add_failed_item(id_=agent_id, error=Cyb3rhqException(CYB3RHQ_EXCEPTION_CODE))
    # Render a valid AffectedItemsCyb3rhqResult and check it has all the expected fields
