...
from cyb3rhq.core.utils import get_date_from_timestamp

with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core import rootcheck
...
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBQuery.__init__")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_init(mock_info, mock_cyb3rhqDBQuery, mock_backend):
    """Test if Cyb3rhqDBQuery and Cyb3rhqDBBackend are called with expected parameters"""
    test = rootcheck.Cyb3rhqDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='test', count=True, get_data=True, distinct=False, filters=None,
                                           fields={})
    mock_backend.assert_called_with('100')
    mock_cyb3rhqDBQuery.assert_called_with(ANY, offset=1, limit=1, table='pm_event', sort=None, search='test',
...
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_default_query(mock_info, mock_backend, distinct):
...
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_parse_filters(mock_info, mock_backend):
...
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_filter_status(mock_info, mock_backend, status, expected_items):
...
@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_filter_status_ko(mock_info, mock_backend):
    """Test if expected exception is raised when status does not exist"""
    test = rootcheck.Cyb3rhqDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='', count=True, get_data=True, distinct=False, fields={},
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})

    with pytest.raises(Cyb3rhqException, match=".* 1603 .*"):
        test._filter_status({'value': 'test'})


@patch("cyb3rhq.core.rootcheck.Cyb3rhqDBBackend")
@patch('cyb3rhq.core.agent.Agent.get_basic_information')
def test_Cyb3rhqDBQueryRootcheck_format_data_into_dictionary(mock_info, mock_backend):
    """Test if format_data_into_dictionary() returns expected element"""
    test = rootcheck.Cyb3rhqDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test',
                                           select=['log', 'date_first', 'status', 'date_last', 'cis', 'pci_dss'],
                                           query='', count=True, get_data=True, distinct=False,
                                           fields=rootcheck.Cyb3rhqDBQueryRootcheck.fields,
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})
...
@patch('cyb3rhq.core.wdb.Cyb3rhqDBConnection')
def test_rootcheck_delete_agent(mock_db_conn, agent):
    """Test if proper parameters are being sent to the cyb3rhqdb socket.

    Parameters
    ----------
    agent : str
        Agent whose information is being deleted from the db.
    mock_db_conn : Cyb3rhqDBConnection
        Object used to send the delete message to the cyb3rhqdb socket.
    """
