...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a Cyb3rhqResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('cyb3rhq.core.security.change_keypair', side_effect=None):
        security, Cyb3rhqResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='cyb3rhq')
        with patch("cyb3rhq.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('cyb3rhq.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('cyb3rhq.core.security.TokenManager.add_user_roles_rules')
...
@patch("cyb3rhq.core.security.revoke_tokens")
@patch("cyb3rhq.core.security.check_database_integrity")
@patch("cyb3rhq.core.security.os.remove")
