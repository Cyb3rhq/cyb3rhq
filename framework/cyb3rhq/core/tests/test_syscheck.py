...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core import syscheck
        from cyb3rhq.core.utils import get_date_from_timestamp


@pytest.mark.parametrize('agent', ['002', '080'])
@patch("cyb3rhq.core.syscheck.Cyb3rhqDBBackend")
@patch("cyb3rhq.core.syscheck.Cyb3rhqDBQuery.__init__")
def test_cyb3rhq_db_query_syscheck__init__(mock_wdbquery, mock_backend, agent):
    """Test if Cyb3rhqDBQuery and Cyb3rhqDBBackend are called with the expected parameters.
    ...
    agent: str
        The agent to test.
    """
    syscheck.Cyb3rhqDBQuerySyscheck(agent)
...
@patch("cyb3rhq.core.syscheck.Cyb3rhqDBBackend")
def test_cyb3rhq_db_syscheck_format_data_into_dictionary(mock_backend, data, is_json):
    """Test if _format_data_into_dictionary() returns the expected element."""
    test = syscheck.Cyb3rhqDBQuerySyscheck('002', offset=0, limit=1000, sort=None, search='test',
