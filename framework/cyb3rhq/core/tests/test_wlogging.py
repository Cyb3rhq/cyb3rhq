...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core import wlogging
...
def test_sizebasedfilerotatinghandler_compute_log_directory(mock_mkdir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("cyb3rhq.core.wlogging.os.path.exists", new=_mock_exists):
...
@patch('cyb3rhq.core.wlogging.SizeBasedFileRotatingHandler')
@patch('cyb3rhq.core.wlogging.TimeBasedFileRotatingHandler')
def test_cyb3rhq_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
                                   max_size):
    """Test if method setup_logger of Cyb3rhqLogger setups the logger attribute properly.
    ...
    # To bypass the checking of the existence of a valid Cyb3rhq install
    # Check time handler
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.Cyb3rhqLogger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.Cyb3rhqLogger, 'log_path', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'logger_name', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'custom_formatter', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Cyb3rhqLogger, 'max_size', create=True, new_callable=PropertyMock)
@patch('logging.Formatter')
def test_cyb3rhq_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
                              mock_foreground_mode, mock_logger, mock_tag, mock_log_path):
    """Test if Cyb3rhqLogger __init__ method initialize all attributes properly."""
    # To bypass the checking of the existence of a valid Cyb3rhq install
    with patch('os.path.join'):
        wlogging.Cyb3rhqLogger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
@patch('cyb3rhq.core.wlogging.SizeBasedFileRotatingHandler')
@patch('cyb3rhq.core.wlogging.TimeBasedFileRotatingHandler')
def test_cyb3rhq_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    """Test if Cyb3rhqLogger __getattr__ method works properly."""
    tmp_dir = tempfile.TemporaryDirectory()
    # To bypass the checking of the existence of a valid Cyb3rhq install
    with patch('os.path.join'):
        w_logger = wlogging.Cyb3rhqLogger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
...
@pytest.mark.parametrize('value, expected', [
    ('Example log', True),
    ('Cyb3rhq Internal Error', False),
    ('Cyb3rhqInternalError', False),
    ('Cyb3rhqError', True),
    ('InternalError', True)
])
