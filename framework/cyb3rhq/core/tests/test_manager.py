...
with patch('cyb3rhq.core.common.cyb3rhq_uid'):
    with patch('cyb3rhq.core.common.cyb3rhq_gid'):
        from cyb3rhq.core.manager import *
...
@patch('cyb3rhq.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'cyb3rhq-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch("cyb3rhq.core.manager.get_cyb3rhq_active_logging_format", return_value=log_format):
        with pytest.raises(Cyb3rhqInternalError, match=".*1020.*"):
            get_ossec_logs()

        with patch('cyb3rhq.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('cyb3rhq.core.manager.tail', return_value=logs):
        ...
        assert result['cyb3rhq-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('cyb3rhq.core.manager.exists', return_value=True)
@patch('cyb3rhq.core.manager.Cyb3rhqSocket')
def test_validate_ossec_conf(mock_cyb3rhqsocket, mock_exists):
    with patch('socket.socket') as sock:
        # Mock sock response
        json_response = json.dumps({'error': 0, 'message': ""}).encode()
        mock_cyb3rhqsocket.return_value.receive.return_value = json_response
        result = validate_ossec_conf()

        assert result == {'status': 'OK'}
        mock_exists.assert_called_with(os.path.join(common.CYB3RHQ_PATH, 'queue', 'sockets', 'com'))
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch("cyb3rhq.core.manager.exists", return_value=False):
            with pytest.raises(Cyb3rhqInternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('cyb3rhq.core.manager.Cyb3rhqSocket.send', side_effect=socket.error):
                with pytest.raises(Cyb3rhqInternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('cyb3rhq.core.manager.Cyb3rhqSocket.receive', side_effect=socket.timeout):
                    with pytest.raises(Cyb3rhqInternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('cyb3rhq.core.manager.Cyb3rhqSocket'):
                    with patch('cyb3rhq.core.manager.parse_execd_output', side_effect=KeyError):
                        with pytest.raises(Cyb3rhqInternalError, match='.* 1904 .*'):
                            validate_ossec_conf()
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(Cyb3rhqException, match=f'.* 1908 .*'):
            parse_execd_output(json_response)


@patch('cyb3rhq.core.manager.configuration.api_conf', new={'experimental_features': True})
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f"v{cyb3rhq.__version__}"
...
async def test_query_update_check_service_request(
    ...
    version = '4.8.0'
    with patch('framework.cyb3rhq.core.manager.cyb3rhq.__version__', version):
        ...
        client_session_get_mock.assert_called_with(
            RELEASE_UPDATES_URL,
            headers={
                CYB3RHQ_UID_KEY: installation_uid,
                CYB3RHQ_TAG_KEY: f'v{version}',
                USER_AGENT_KEY: f'Cyb3rhq UpdateCheckService/v{version}'
            },
