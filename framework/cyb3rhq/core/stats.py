...
from cyb3rhq.core import common, utils
from cyb3rhq.core import cyb3rhq_socket
from cyb3rhq.core.exception import Cyb3rhqError, Cyb3rhqInternalError, Cyb3rhqException
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise Cyb3rhqError(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise Cyb3rhqInternalError(1309)
...
async def get_daemons_stats_socket(socket: str,
    ...
    """Send message to Cyb3rhq socket to get statistical information.
    ...
    """
    # Create message
    full_message = cyb3rhq_socket.create_cyb3rhq_socket_message(
    ...
    # Connect to socket
    try:
        s = cyb3rhq_socket.Cyb3rhqAsyncSocketJSON()
        await s.connect(socket)
    except Exception as exc:
        raise Cyb3rhqInternalError(1121, extra_message=socket) from exc
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise Cyb3rhqInternalError(1104, extra_message=str(e))
    except IOError:
        raise Cyb3rhqError(1308, extra_message=filename)
...
def get_stats_socket_path(agent_id: Union[str, int], daemon: str) -> str:
    ...
    if is_agent_a_manager(agent_id):
        return os.path.join(common.CYB3RHQ_PATH, "queue", "sockets", daemon)
    else:
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    except Cyb3rhqException:
        # Error connecting to socket
        raise Cyb3rhqInternalError(1121)
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise Cyb3rhqError(1117, extra_message=rec_msg)
