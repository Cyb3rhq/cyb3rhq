...
from cyb3rhq.core import common, configuration, stats
from cyb3rhq.core.InputValidator import InputValidator
from cyb3rhq.core.cluster.utils import get_manager_status
from cyb3rhq.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from cyb3rhq.core.exception import Cyb3rhqException, Cyb3rhqError, Cyb3rhqInternalError, Cyb3rhqResourceNotFound
from cyb3rhq.core.utils import Cyb3rhqVersion, plain_dict_to_nested_dict, get_fields_to_nest, Cyb3rhqDBQuery, \
    Cyb3rhqDBQueryDistinct, Cyb3rhqDBQueryGroupBy, Cyb3rhqDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from cyb3rhq.core.cyb3rhq_queue import Cyb3rhqQueue
from cyb3rhq.core.cyb3rhq_socket import Cyb3rhqSocket, Cyb3rhqSocketJSON, create_cyb3rhq_socket_message
...
class Cyb3rhqDBQueryAgents(Cyb3rhqDBQuery):
    """Class used to query Cyb3rhq agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_cyb3rhq_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = Cyb3rhqDBBackend(query_format='global')
        Cyb3rhqDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Cyb3rhq query."""
        Cyb3rhqDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return Cyb3rhqDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Cyb3rhq query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            Cyb3rhqDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise Cyb3rhqError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            Cyb3rhqDBQuery._process_filter(self, field_name, field_filter, q_filter)


class Cyb3rhqDBQueryGroup(Cyb3rhqDBQuery):
    """Class used to query Cyb3rhq groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = Cyb3rhqDBBackend(query_format='global')
        Cyb3rhqDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class Cyb3rhqDBQueryDistinctAgents(Cyb3rhqDBQueryDistinct, Cyb3rhqDBQueryAgents):
    pass


class Cyb3rhqDBQueryGroupByAgents(Cyb3rhqDBQueryGroupBy, Cyb3rhqDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        Cyb3rhqDBQueryAgents.__init__(self, *args, **kwargs)
        Cyb3rhqDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return Cyb3rhqDBQuery._format_data_into_dictionary(self)


class Cyb3rhqDBQueryMultigroups(Cyb3rhqDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        Cyb3rhqDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        Cyb3rhqResourceNotFound(1701)
            Agent does not exist.
        """
        with Cyb3rhqDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: Cyb3rhqQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : Cyb3rhqQueue
            Cyb3rhqQueue used for the active response message.
        ...
        # Send force reconnect message to the Cyb3rhqQueue
        ret_msg = wq.send_msg_to_agent(Cyb3rhqQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Cyb3rhq.
        """
        # Check that cyb3rhq-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except Cyb3rhqInternalError as e:
            # cyb3rhq-authd is not running due to a problem with /proc availability
            raise Cyb3rhqError(1726, extra_message=str(e))

        if manager_status.get('cyb3rhq-authd') != 'running':
            # cyb3rhq-authd is not running
            raise Cyb3rhqError(1726)
        ...
        try:
            ...
            raise Cyb3rhqInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = Cyb3rhqSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise Cyb3rhqError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise Cyb3rhqInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Cyb3rhq using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise Cyb3rhqError(1705, extra_message=name)
            elif e.code == 9007:
                raise Cyb3rhqError(1706, extra_message=ip)
            elif e.code == 9012:
                raise Cyb3rhqError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = Cyb3rhqDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = Cyb3rhqDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise Cyb3rhqResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise Cyb3rhqError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise Cyb3rhqError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Cyb3rhq vX.Y.Z.
        ...
        """
        if Cyb3rhqVersion(agent_version) < Cyb3rhqVersion(common.ACTIVE_CONFIG_VERSION):
            raise Cyb3rhqInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        Cyb3rhqInternalError(1015)
            Agent version is null.
        Cyb3rhqInternalError(1735)
        ...
        if self.version is None:
            raise Cyb3rhqInternalError(1015)
        agent_version = Cyb3rhqVersion(self.version.split(" ")[1])
        required_version = Cyb3rhqVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise Cyb3rhqInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_cyb3rhq_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_cyb3rhq_version_format(filters: dict):
    """Verify and format the specified cyb3rhq version into the 'cyb3rhq vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"cyb3rhq {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^cyb3rhq \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: Cyb3rhqQueue = None) -> str:
    ...
    wq : Cyb3rhqQueue
        Cyb3rhqQueue used for the active response messages.
    ...
    str
        Message generated by Cyb3rhq.
    """
    # If the Cyb3rhq agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if Cyb3rhqVersion(agent_version) >= Cyb3rhqVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(Cyb3rhqQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(Cyb3rhqQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_cyb3rhq_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_cyb3rhq_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = Cyb3rhqSocket(common.UPGRADE_SOCKET)
