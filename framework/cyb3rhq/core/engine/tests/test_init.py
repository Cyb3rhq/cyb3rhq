...
from cyb3rhq.core.engine import DEFAULT_RETRIES, DEFAULT_TIMEOUT, Engine, get_engine_client
...
async def test_get_engine_client_ko(socket_path: str, error_number: int):
    """Check that the `get_engine_client` returns a Cyb3rhqEngineError on an exception."""
    with pytest.raises(Cyb3rhqEngineError, match=f'.*{error_number}.*'):
        async with get_engine_client() as engine:
