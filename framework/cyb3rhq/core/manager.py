...
from cyb3rhq.core.configuration import get_active_configuration, get_cti_url
from cyb3rhq.core.utils import get_utc_now, get_utc_strptime, tail
from cyb3rhq.core.cyb3rhq_socket import Cyb3rhqSocket
...
CYB3RHQ_UID_KEY = 'cyb3rhq-uid'
CYB3RHQ_TAG_KEY = 'cyb3rhq-tag'
...
def get_ossec_log_fields(log: str, log_format: LoggingFormat = LoggingFormat.plain) -> Union[tuple, None]:
    ...
    if "rootcheck" in tag:  # Unify rootcheck category
        tag = "cyb3rhq-rootcheck"

    return get_utc_strptime(date, '%Y/%m/%d %H:%M:%S'), tag, level.lower(), description


def get_cyb3rhq_active_logging_format() -> LoggingFormat:
    """Obtain the Cyb3rhq active logging format.

    Returns
    -------
    LoggingFormat
        Cyb3rhq active log format. Can either be `plain` or `json`. If it has both types, `plain` will be returned.
    """
...
def get_ossec_logs(limit: int = 2000) -> list:
    ...
    log_format = get_cyb3rhq_active_logging_format()
    if log_format == LoggingFormat.plain and exists(common.CYB3RHQ_LOG):
        cyb3rhq_log_content = tail(common.CYB3RHQ_LOG, limit)
    elif log_format == LoggingFormat.json and exists(common.CYB3RHQ_LOG_JSON):
        cyb3rhq_log_content = tail(common.CYB3RHQ_LOG_JSON, limit)
    else:
        raise Cyb3rhqInternalError(1020)

    for line in cyb3rhq_log_content:
...
def validate_ossec_conf() -> str:
    ...
    # Message for checking Cyb3rhq configuration
    wcom_msg = common.CHECK_CONFIG_COMMAND

    # Connect to wcom socket
    if exists(wcom_socket_path):
        try:
            wcom_socket = Cyb3rhqSocket(wcom_socket_path)
        except Cyb3rhqException as e:
            extra_msg = f'Socket: CYB3RHQ_PATH/queue/sockets/com. Error {e.message}'
            raise Cyb3rhqInternalError(1013, extra_message=extra_msg)
    else:
    ...
    try:
        ...
        raise Cyb3rhqInternalError(1904, extra_message=str(e))
...
def parse_execd_output(output: str) -> Dict:
    ...
    if error_flag != 0:
        ...
        raise Cyb3rhqError(1908, extra_message=', '.join(errors))
    else:
        response = {'status': 'OK'}
...
def get_update_information_template(
        uuid: str,
        update_check: bool,
        current_version: str = f"v{cyb3rhq.__version__}",
    ...
    current_version : str, optional
        Indicates the current version of Cyb3rhq, by default cyb3rhq.__version__.
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        Cyb3rhq UID to include in the query.
    ...
    update_information : dict
        Updates information.
    """
    current_version = f'v{cyb3rhq.__version__}'
    headers = {
        CYB3RHQ_UID_KEY: installation_uid,
        CYB3RHQ_TAG_KEY: current_version,
        USER_AGENT_KEY: f'Cyb3rhq UpdateCheckService/{current_version}'
    }
