...
def create_pid(name: str, pid: int):
    ...
    Raises
    ------
    Cyb3rhqInternalError(3002)
        Error creating pidfile.
    """
    filename = path.join(common.CYB3RHQ_PATH, common.OS_PIDFILE_PATH, f'{name}-{pid}.pid')
    ...
    with open(filename, 'a') as fp:
        try:
            ...
            raise Cyb3rhqInternalError(3002, str(e))
...
def delete_child_pids(name: str, ppid: int, logger: logging.Logger):
    ...
    logger : logging.Logger
        Logger object.
    """
    filenames = glob.glob(path.join(common.CYB3RHQ_PATH, common.OS_PIDFILE_PATH, f'{name}*.pid'))
