...
import cyb3rhq.core.results as results
...
def assign_cyb3rhq_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.cyb3rhq_gid() or \
        os.stat(filepath).st_uid != common.cyb3rhq_uid():
        os.chown(filepath, common.cyb3rhq_uid(), common.cyb3rhq_gid())
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.CYB3RHQ_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.CYB3RHQ_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Cyb3rhq framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise Cyb3rhqError(1405, extra_message=str(limit))
        elif limit == 0:
            raise Cyb3rhqError(1406)
    ...
    if offset < 0:
        raise Cyb3rhqError(1400)
    elif limit < 1:
        raise Cyb3rhqError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise Cyb3rhqError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'cyb3rhq' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise Cyb3rhqError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise Cyb3rhqError(1724, "{}".format(', '.join(select)))
...
def delete_cyb3rhq_file(full_path: str) -> bool:
    """Delete a Cyb3rhq file.
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.CYB3RHQ_PATH) or '..' in full_path:
        raise Cyb3rhqError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise Cyb3rhqError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_cyb3rhq_limits_unchanged(new_conf, original_conf):
    """Check if Cyb3rhq limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise Cyb3rhqError(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise Cyb3rhqError(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise Cyb3rhqError(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise Cyb3rhqError(1130, extra_message='integrations > virustotal')


def load_cyb3rhq_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise Cyb3rhqError(1113, extra_message=str(e))
...
class Cyb3rhqVersion:

    def __init__(self, version):

        pattern = r"(?:Cyb3rhq )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise Cyb3rhqError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise Cyb3rhqError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class Cyb3rhqDBBackend(AbstractDatabaseBackend):
    """
    This class describes a cyb3rhq db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise Cyb3rhqError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return Cyb3rhqDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through Cyb3rhqDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class Cyb3rhqDBQuery(object):
    """This class describes a database query for cyb3rhq."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        # For example, the following regex -> (name!=cyb3rhq;id>5),group=webserver <- would return 3 different matches:
        #   (name != cyb3rhq ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, Cyb3rhqDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise Cyb3rhqError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise Cyb3rhqError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not set_select_fields or not set_select_fields.issubset(set_fields_keys):
                raise Cyb3rhqError(1724, "Allowed select fields: {0}. Fields {1}". \
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise Cyb3rhqError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'Cyb3rhqDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'Cyb3rhqDBQueryGroups':
            resource = 'name'
        else:
            raise Cyb3rhqInternalError(1123)
...
class Cyb3rhqDBQueryDistinct(Cyb3rhqDBQuery):
    ...
    def _add_filters_to_query(self):
        Cyb3rhqDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise Cyb3rhqError(1410)

        Cyb3rhqDBQuery._add_select_to_query(self)
...
def validate_cyb3rhq_xml(content: str, config_file: bool = False):
    """Validate Cyb3rhq XML files (rules, decoders and ossec.conf)
    ...
    try:
        ...
        if config_file:
            ...
            check_cyb3rhq_limits_unchanged(final_xml, current_xml)
        # Check xml format
        load_cyb3rhq_xml(xml_path='', data=final_xml)
    except ExpatError:
        raise Cyb3rhqError(1113)
    except Cyb3rhqError as e:
        raise e
    except Exception as e:
        raise Cyb3rhqError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    Cyb3rhqInternalError(1005)
        Error reading file.
    Cyb3rhqInternalError(1016)
        Error moving file.
    Cyb3rhqError(1006)
    ...
    try:
        ...
        raise Cyb3rhqInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.CYB3RHQ_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.cyb3rhq_uid(), common.cyb3rhq_gid()), permissions=0o660)
    except PermissionError as exc:
        raise Cyb3rhqError(1006) from exc
    except Error as exc:
        raise Cyb3rhqInternalError(1016) from exc

    return results.Cyb3rhqResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.CYB3RHQ_PATH) -> str:
    """Return a relative path from the Cyb3rhq base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.CYB3RHQ_PATH`
