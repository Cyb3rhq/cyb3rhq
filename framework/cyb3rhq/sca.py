...
from cyb3rhq.core.sca import (
    Cyb3rhqDBQueryDistinctSCACheck, Cyb3rhqDBQuerySCA, Cyb3rhqDBQuerySCACheck, Cyb3rhqDBQuerySCACheckIDs,
    Cyb3rhqDBQuerySCACheckRelational, SCA_CHECK_COMPLIANCE_DB_FIELDS, SCA_CHECK_RULES_DB_FIELDS, SCA_CHECK_DB_FIELDS)
...
def get_sca_list(agent_list: list = None, q: str = "", offset: int = 0, limit: int = common.DATABASE_LIMIT,
                 ...
                 filters: dict = None) -> AffectedItemsCyb3rhqResult:
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='All selected SCA information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            with Cyb3rhqDBQuerySCA(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,
            ...
            result.add_failed_item(id_=agent_list[0], error=Cyb3rhqResourceNotFound(1701))
...
def get_sca_checks(policy_id: str = None, agent_list: list = None, q: str = "", offset: int = 0,
                   ...
                   filters: dict = None, distinct: bool = False) -> AffectedItemsCyb3rhqResult:
    ...
    Cyb3rhqInternalError(2007)
        If there was an error retrieving data from Cyb3rhq DB.
    ...
    AffectedItemsCyb3rhqResult
        Affected items.
    """
    result = AffectedItemsCyb3rhqResult(all_msg='All selected SCA/policy information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            if not distinct:
                ...
                with Cyb3rhqDBQuerySCACheckIDs(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
                ...
                if id_check_list:
                    ...
                    if not select or select_compliance:
                        with Cyb3rhqDBQuerySCACheckRelational(
            ...
            # Workaround for distinct=True
                ...
                with Cyb3rhqDBQueryDistinctSCACheck(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
