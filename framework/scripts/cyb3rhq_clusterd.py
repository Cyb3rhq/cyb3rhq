...
from cyb3rhq.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> Cyb3rhqLogger:
    """Set cluster logger.
...
def print_version():
    """Print Cyb3rhq metadata."""
    from cyb3rhq.core.cluster import __author__, __licence__, __version__, __cyb3rhq_name__
    print(f"\n{__cyb3rhq_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('cyb3rhq-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('cyb3rhq-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: Cyb3rhqLogger):
    ...
    logger : Cyb3rhqLogger
        Cluster logger.
    """
    from cyb3rhq.core.cluster import local_server, master
    from cyb3rhq.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: Cyb3rhqLogger):
    ...
    from cyb3rhq.core.cluster import local_server, worker
...
def main():
    """Main function of the cyb3rhq-clusterd script in charge of starting the cluster process."""
    import cyb3rhq.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.CYB3RHQ_PATH}/logs/cluster.log'):
        os.chown(f'{common.CYB3RHQ_PATH}/logs/cluster.log', common.cyb3rhq_uid(), common.cyb3rhq_gid())
        os.chmod(f'{common.CYB3RHQ_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        cyb3rhq.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    cyb3rhq.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('cyb3rhq-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to cyb3rhq
    if not args.root:
        os.setgid(common.cyb3rhq_gid())
        os.setuid(common.cyb3rhq_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('cyb3rhq-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('cyb3rhq-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('cyb3rhq-clusterd', pid)


if __name__ == '__main__':
    import cyb3rhq.core.cluster.utils as cluster_utils
    from cyb3rhq.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('cyb3rhq_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
