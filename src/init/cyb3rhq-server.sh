...
# cyb3rhq-control        This shell script takes care of starting
...
AUTHOR="Cyb3rhq Inc."
USE_JSON=false
DAEMONS="cyb3rhq-clusterd cyb3rhq-modulesd cyb3rhq-monitord cyb3rhq-logcollector cyb3rhq-remoted cyb3rhq-syscheckd cyb3rhq-analysisd cyb3rhq-maild cyb3rhq-execd cyb3rhq-db cyb3rhq-authd cyb3rhq-agentlessd cyb3rhq-integratord cyb3rhq-dbd cyb3rhq-csyslogd cyb3rhq-apid cyb3rhq-comms-apid"
OP_DAEMONS="cyb3rhq-clusterd cyb3rhq-maild cyb3rhq-agentlessd cyb3rhq-integratord cyb3rhq-dbd cyb3rhq-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Cyb3rhq $VERSION..."
    fi

    TEST=$(${DIR}/bin/cyb3rhq-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/cyb3rhq-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If cyb3rhq-maild is disabled, don't try to start it.
        if [ X"$i" = "Xcyb3rhq-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If cyb3rhq-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xcyb3rhq-clusterd" ]; then
        ...
        ## If cyb3rhq-authd is disabled, don't try to start it.
        if [ X"$i" = "Xcyb3rhq-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Cyb3rhq, removing..."
                fi
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"CYB3RHQ_VERSION":"'${VERSION}'"},'
            echo -n '{"CYB3RHQ_REVISION":"'${REVISION}'"},'
            echo -n '{"CYB3RHQ_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "CYB3RHQ_VERSION=\"${VERSION}\""
            echo "CYB3RHQ_REVISION=\"${REVISION}\""
            echo "CYB3RHQ_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/cyb3rhq-execd//')
    restart_service
    ;;
status)
    lock
