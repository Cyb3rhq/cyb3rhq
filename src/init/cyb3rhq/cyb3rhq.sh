...
# Install functions for Cyb3rhq
# wazuh.com (https://github.com/cyb3rhq)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/cyb3rhq.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/cyb3rhq.pp /tmp && semodule -i /tmp/cyb3rhq.pp
                rm -f /tmp/cyb3rhq.pp
                semodule -e cyb3rhq
            fi
        fi
...
Cyb3rhqUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown cyb3rhq:cyb3rhq $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Cyb3rhq DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/cyb3rhq
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Cyb3rhq tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:cyb3rhq
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown cyb3rhq:cyb3rhq
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:cyb3rhq {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown cyb3rhq:cyb3rhq {} \;
        fi
    fi
