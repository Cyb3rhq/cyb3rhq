...
AUTHOR="Cyb3rhq Inc."
DAEMONS="cyb3rhq-modulesd cyb3rhq-monitord cyb3rhq-logcollector cyb3rhq-syscheckd cyb3rhq-analysisd cyb3rhq-maild cyb3rhq-execd cyb3rhq-db cyb3rhq-agentlessd cyb3rhq-integratord cyb3rhq-dbd cyb3rhq-csyslogd"
...
start_service()
{
    ...
    TEST=$(${DIR}/bin/cyb3rhq-logtest-legacy -t  2>&1)
    echo $TEST

    if [ ! -z "$TEST" ]; then
        echo "cyb3rhq-analysisd: Configuration error. Exiting."
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/cyb3rhq-control start
    fi
...
stop_service()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/cyb3rhq-control stop
    fi
