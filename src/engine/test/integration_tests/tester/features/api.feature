Feature: Tester API Management
  ...
  Scenario: Add a repeated session for testing via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "cyb3rhq-core-test" loaded
    And I create a "default" session that points to policy "policy/cyb3rhq/0"
    When I send a request to the tester to add a new session called "default" with the data from policy:"policy/cyb3rhq/0"
  ...
  Scenario: Add a new session for testing via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "cyb3rhq-core-test" loaded
    And I create a "default" session that points to policy "policy/cyb3rhq/0"
    When I send a request to the tester to add a new session called "test1" with the data from policy:"policy/cyb3rhq/0"
    Then I should receive a success response

  Scenario: Remove specific session via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "cyb3rhq-core-test" loaded
    And I create a "default" session that points to policy "policy/cyb3rhq/0"
    When I send a request to the tester to add 5 sessions called "dummy" with policy "policy/cyb3rhq/0"
  ...
  Scenario: Change sync to OUTDATED of specific session via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "cyb3rhq-core-test" loaded
    And I create a "default" session that points to policy "policy/cyb3rhq/0"
    When I send a request to the policy "policy/cyb3rhq/0" to add an integration called "other-cyb3rhq-core-test"
  ...
  Scenario: Change sync to ERROR of specific session via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "cyb3rhq-core-test" loaded
    And I create a "default" session that points to policy "policy/cyb3rhq/0"
    When I send a request to delete the policy "policy/cyb3rhq/0"
    And I send a request to the tester to get the session "default"
  ...
  Scenario: Send events to specific session without debug session via API
    Given I have a policy "policy/cyb3rhq/0" that has an integration called "other-cyb3rhq-core-test" loaded
    And I create a "test" session that points to policy "policy/cyb3rhq/0"
    ...
    Then I should receive the next output: "{"output": {"event": {"original": "hi! i am an event test!"}, "cyb3rhq": {"location": "any", "queue": 50.0}}}"
  ...
  Scenario: Send events to specific session with low debug via API
    ...
    Then I should receive the next output: "{"output": {"cyb3rhq": {"location": "any", "queue": 50.0}, "event": {"original": "hi! i am an event test!"}}, "assetTraces": [{"asset": "decoder/other-test-message/0", "success": true}]}"
  ...
  Scenario: Send events to specific session with high debug via API
    ...
    Then I should receive the next output: "{"output": {"event": {"original": "hi! i am an event test!"}, "cyb3rhq": {"queue": 50.0, "location": "any"}}, "assetTraces": [{"asset": "decoder/other-test-message/0", "success": true, "traces": ["[check: $cyb3rhq.queue == 50] -> Success"]}]}"
