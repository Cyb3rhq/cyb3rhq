#ifndef _BASE_UTILS_CYB3RHQ_RESPONSE_HPP
#define _BASE_UTILS_CYB3RHQ_RESPONSE_HPP
...
public:
    ...
    /**
     * @brief  Construct a new Cyb3rhq Response object
     *
     * @param data Data to be sent, it can be a json object or a string
     * @param error Error code (0 if no error)
     * @param message Optional message
     */
    explicit Cyb3rhqResponse(const json::Json& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
     ...
     */
    explicit Cyb3rhqResponse(json::Json&& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
        m_message = message.empty() ? std::nullopt : std::optional<std::string> {message};
     ...
     */
    explicit Cyb3rhqResponse(std::string_view message) noexcept
        : m_data(json::Json {R"({})"})
     ...
     * @param message Messsage to be sent as a response
     */
    explicit Cyb3rhqResponse(json::Json& data, std::string_view message) noexcept
        : m_data(data)
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message(message) {};
     ...
     */
    explicit Cyb3rhqResponse() noexcept
        : m_data(json::Json {R"({})"})
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message() {};
    ...
    /**
     * @brief Create a Cyb3rhqResponse object from a string
     *
     * @param response Response as a string.
     * @return Cyb3rhqResponse object.
     ...
     * }
     */
    static Cyb3rhqResponse fromStr(std::string_view response)
    {
        ...
        auto ret = Cyb3rhqResponse {std::move(data.value()), error.value()};
     ...
     *
     */
    static Cyb3rhqResponse invalidJsonRequest()
    {
        return Cyb3rhqResponse(json::Json(R"({})"),
     ...
     *
     */
    static Cyb3rhqResponse invalidSize()
    {
        return Cyb3rhqResponse(
     ...
     *
     */
    static Cyb3rhqResponse unknownError()
    {
        return Cyb3rhqResponse(
     ...
     *
     */
    static Cyb3rhqResponse invalidRequest(const std::string& message)
    {
        return Cyb3rhqResponse(json::Json(R"({})"),
     ...
     * process  the respons
     */
    static Cyb3rhqResponse internalError(const std::string& message)
    {
        return Cyb3rhqResponse(json::Json(R"({})"),
     ...
     *
     */
    static Cyb3rhqResponse busyServer()
    {
        return Cyb3rhqResponse(
...
#endif // _API_CYB3RHQ_RESPONSE_HPP
