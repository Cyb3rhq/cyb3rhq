...
if __name__ == '__main__':
    try:
        arg_parser = ArgumentParser(
            description='Compare expected results of an output and cyb3rhq output. Files should be a json consisting of an array of output objects, the script expects that the order of both files is the same')
        arg_parser.add_argument(
            'table', help='CSV table with metadata about the test')
        arg_parser.add_argument(
            'cyb3rhq_output', type=Path, help='Path to the file with cyb3rhq output')
        arg_parser.add_argument(
        ...
        table = args.table
        cyb3rhq_output_path = Path(args.cyb3rhq_output)
        ...
        # Read the files
        cyb3rhq_expected = loads(cyb3rhq_output_path.read_text())
        other_expected = loads(other_output_path.read_text())

        # Verify that the number of outputs is the same
        if len(cyb3rhq_expected) != len(other_expected):
            raise Exception(
                f'Expected the same number of outputs, got {len(cyb3rhq_expected)} cyb3rhq outputs and {len(other_expected)} other outputs')

        # Compare each output following the table rules
        compare_results = []

        for cyb3rhq_output, other_output in zip(cyb3rhq_expected, other_expected):
            current_result = {}
            current_result['original'] = get_value(
                'event.original', cyb3rhq_output)
            checked_fields = set()
            checked_fields.add('event.original')

            # Need to match
            for field in fields_needed_to_match(df):
                waz_value = get_value(field, cyb3rhq_output)
            ...
            for field in fields_mapped_by_waz(df):
                ...
                # Cyb3rhq extra fields
                if waz_value is not None and oth_value is None and show_extra:
                    if 'cyb3rhq_extra' not in current_result:
                        current_result['cyb3rhq_extra'] = []
                    current_result['cyb3rhq_extra'].append(field)
        ...
        # Notify the user
        print(
            f'{cyb3rhq_output_path.name} and {other_output_path.name} -> {output_path}')
