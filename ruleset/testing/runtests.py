...
def getCyb3rhqInfo(cyb3rhq_home):
    cyb3rhq_control = os.path.join(cyb3rhq_home, "bin", "cyb3rhq-control")
    cyb3rhq_env_vars = {}
    try:
        proc = subprocess.Popen([cyb3rhq_control, "info"], stdout=subprocess.PIPE)
        (stdout, stderr) = proc.communicate()
    except Exception as e:
        print("Seems like there is no Cyb3rhq installation.")
        return None
...
def provisionDR():
    ...
    for file in os.listdir(rules_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?rules.xml$', file):
            shutil.copy2(file_fullpath, args.cyb3rhq_home + "/etc/rules")
    ...
    for file in os.listdir(decoders_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?decoders.xml$', file):
            shutil.copy2(file_fullpath, args.cyb3rhq_home + "/etc/decoders")



def cleanDR():
    rules_dir = args.cyb3rhq_home + "/etc/rules"
    decoders_dir = args.cyb3rhq_home + "/etc/decoders"
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/cyb3rhq-logtest' % (self._ossec_path), ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests Cyb3rhq rules.')
    parser.add_argument('--path', '-p', default='/var/ossec', dest='cyb3rhq_home',
                        help='Use -p or --path to specify Cyb3rhq installation path')
    ...
    cyb3rhq_info = getCyb3rhqInfo(args.cyb3rhq_home)
    if cyb3rhq_info is None:
        sys.exit(1)
    ...
    provisionDR()
    OT = OssecTester(args.cyb3rhq_home)
