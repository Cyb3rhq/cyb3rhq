...
from cyb3rhq_testing.constants.daemons import CYB3RHQ_MANAGER, API_DAEMONS_REQUIREMENTS
from cyb3rhq_testing.constants.paths import ROOT_PREFIX
from cyb3rhq_testing.constants.paths.api import RBAC_DATABASE_PATH
from cyb3rhq_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, CYB3RHQ_LOG_PATH, ALERTS_JSON_PATH, \
                                               CYB3RHQ_API_LOG_FILE_PATH, CYB3RHQ_API_JSON_LOG_FILE_PATH
...
from cyb3rhq_testing.tools import socket_controller
from cyb3rhq_testing.tools.monitors import queue_monitor
from cyb3rhq_testing.tools.monitors.file_monitor import FileMonitor
from cyb3rhq_testing.tools.simulators.agent_simulator import create_agents, connect
...
from cyb3rhq_testing.utils import configuration, database, file, mocking, services
from cyb3rhq_testing.utils.file import remove_file, truncate_file
from cyb3rhq_testing.utils.manage_agents import remove_agents
import cyb3rhq_testing.utils.configuration as cyb3rhq_configuration
...
@pytest.fixture()
def backup_cyb3rhq_configuration() -> None:
    """Backup cyb3rhq configuration. Saves the initial configuration and restores it after the test."""
    # Save configuration
...
def configure_local_internal_options_function(request):
    ...
    backup_local_internal_options = cyb3rhq_configuration.get_local_internal_options_dict()

    logger.debug(f"Set local_internal_option to {str(local_internal_options)}")
    cyb3rhq_configuration.set_local_internal_options_dict(local_internal_options)

    yield

    logger.debug(f"Restore local_internal_option to {str(backup_local_internal_options)}")
    cyb3rhq_configuration.set_local_internal_options_dict(backup_local_internal_options)
...
def truncate_monitored_files_implementation() -> None:
    ...
    if services.get_service() == CYB3RHQ_MANAGER:
        log_files = [CYB3RHQ_LOG_PATH, ALERTS_JSON_PATH, CYB3RHQ_API_LOG_FILE_PATH,
                     CYB3RHQ_API_JSON_LOG_FILE_PATH, CYB3RHQ_CLIENT_KEYS_PATH]
    else:
...
@pytest.fixture(scope='module')
def restart_cyb3rhq_daemon_after_finishing_module(daemon: str = None) -> None:
    """Restart a Cyb3rhq daemons and clears the cyb3rhq log after the test module finishes execution.
    Args:
...
def configure_sockets_environment_implementation(request: pytest.FixtureRequest) -> None:
    ...
    # Stop cyb3rhq-service and ensure all daemons are stopped
    services.control_service('stop')
...
def mock_agent_module():
    """Fixture to create a mocked agent in cyb3rhq databases"""
...
def autostart_simulators(request: pytest.FixtureRequest) -> None:
    ...
    CYB3RHQ_MANAGER, and when the test function is not already using the simulator fixture, if it does
    ...
    if services.get_service() is not CYB3RHQ_MANAGER:
