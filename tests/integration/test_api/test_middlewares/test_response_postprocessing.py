...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'cyb3rhq-apid' daemon is
       working properly. The Cyb3rhq API is an open source 'RESTful' API that allows the interaction with the Cyb3rhq
...
from cyb3rhq_testing.constants.api import CYB3RHQ_API_PROTOCOL
from cyb3rhq_testing.modules.api.utils import login, get_base_url, set_authorization_header
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{CYB3RHQ_API_PROTOCOL}://", adapter)
