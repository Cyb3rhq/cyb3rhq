...
from cyb3rhq_testing.constants.paths.api import CYB3RHQ_API_SCRIPT
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'cyb3rhq') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search cyb3rhq-apid process and verify that it is present
    ...
    expected_output:
        ...
        - cyb3rhq (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get cyb3rhq-apid process info
    api_process = search_process_by_command(CYB3RHQ_API_SCRIPT)
    assert api_process is not None, f"The process {CYB3RHQ_API_SCRIPT} could not be found"
