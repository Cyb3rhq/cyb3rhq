...
from cyb3rhq_testing.constants.api import CONFIGURATION_TYPES, CYB3RHQ_API_USER, LOGIN_ROUTE
from cyb3rhq_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from cyb3rhq_testing.constants.paths.logs import CYB3RHQ_API_JSON_LOG_FILE_PATH, CYB3RHQ_API_LOG_FILE_PATH
from cyb3rhq_testing.modules.api.patterns import API_TIMEOUT_ERROR_MSG, API_LOGIN_REQUEST_MSG
from cyb3rhq_testing.modules.api.utils import login
...
def test_logs_formats(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler):
    ...
    if 'json' in current_formats:
        json_file_monitor = file_monitor.FileMonitor(CYB3RHQ_API_JSON_LOG_FILE_PATH)
        if current_level == 'error':
    ...
    if 'plain' in current_formats:
        plain_file_monitor = file_monitor.FileMonitor(CYB3RHQ_API_LOG_FILE_PATH)
        if current_level == 'error':
