...
from cyb3rhq_testing.constants.paths.logs import CYB3RHQ_API_LOG_FILE_PATH, CYB3RHQ_API_JSON_LOG_FILE_PATH
from cyb3rhq_testing.constants.api import CYB3RHQ_API_PORT, CONFIGURATION_TYPES
from cyb3rhq_testing.modules.api.configuration import get_configuration, append_configuration, delete_configuration_file
...
def add_configuration(test_configuration: list[dict], request: pytest.FixtureRequest) -> None:
    """Add configuration to the Cyb3rhq API configuration files.
...
def wait_for_api_start(test_configuration: dict) -> None:
    ...
    if test_configuration is not None:
        if test_configuration.get('blocks') is not None:
            ...
            port = test_configuration['blocks'].get('port', CYB3RHQ_API_PORT)

    file_to_monitor = CYB3RHQ_API_JSON_LOG_FILE_PATH if logs_format == 'json' else CYB3RHQ_API_LOG_FILE_PATH
