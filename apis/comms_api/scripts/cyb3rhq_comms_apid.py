...
MAIN_PROCESS = 'cyb3rhq-comms-apid'
...
def configure_ssl(keyfile: str, certfile: str) -> None:
    ...
    try:
        ...
        raise Cyb3rhqCommsAPIError(2700, extra_message=str(exc))
    except IOError as exc:
        if exc.errno == 22:
            raise Cyb3rhqCommsAPIError(2701, extra_message=str(exc))
        elif exc.errno == 13:
            raise Cyb3rhqCommsAPIError(2702, extra_message=str(exc))
        else:
            raise Cyb3rhqCommsAPIError(2703, extra_message=str(exc))
...
def get_gunicorn_options(pid: int, foreground_mode: bool, log_config_dict: dict) -> dict:
    ...
    pidfile = os.path.join(common.CYB3RHQ_PATH, common.OS_PIDFILE_PATH, f'{MAIN_PROCESS}-{pid}.pid')
...
if __name__ == '__main__':
    ...
    except Cyb3rhqCommsAPIError as e:
        logger.error(f'Error when trying to start the Cyb3rhq Communications API. {e}')
        exit(1)
    except Exception as e:
        logger.error(f'Internal error when trying to start the Cyb3rhq Communications API. {e}')
        exit(1)
    finally:
