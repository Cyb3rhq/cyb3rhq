...
API_MAIN_PROCESS = 'cyb3rhq-apid'
API_LOCAL_REQUEST_PROCESS = 'cyb3rhq-apid_exec'
API_AUTHENTICATION_PROCESS = 'cyb3rhq-apid_auth'
API_SECURITY_EVENTS_PROCESS = 'cyb3rhq-apid_events'
...
def configure_ssl(params):
    ...
    try:
        ...
        if not os.path.exists(api_conf['https']['key']) \
            ...
            logger.info(
                f"Generated private key file in CYB3RHQ_PATH/{to_relative_path(api_conf['https']['key'])}")
            generate_self_signed_certificate(private_key, api_conf['https']['cert'])
            logger.info(
                f"Generated certificate file in CYB3RHQ_PATH/{to_relative_path(api_conf['https']['cert'])}")
        ...
        # Check and assign ownership to cyb3rhq user for server.key and server.crt files
        utils.assign_cyb3rhq_ownership(api_conf['https']['key'])
        utils.assign_cyb3rhq_ownership(api_conf['https']['cert'])
        ...
        if exc.errno == 22:
            ...
            msg = f'Cyb3rhq API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file CYB3RHQ_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict):
    ...
    If another Cyb3rhq API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(Cyb3rhqAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__cyb3rhq_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by cyb3rhq api and framework."""
...
if __name__ == '__main__':
    ...
    from cyb3rhq.core import common, pyDaemonModule, utils
    ...
    try:
        ...
        print(f"Error when trying to start the Cyb3rhq API. {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error when trying to start the Cyb3rhq API. {api_log_error}")
        sys.exit(1)

    # set permission on log files
    for handler in uvicorn_params['log_config']['handlers'].values():
        if 'filename' in handler:
            utils.assign_cyb3rhq_ownership(handler['filename'])
            os.chmod(handler['filename'], 0o660)

    # Configure and create the cyb3rhq-api logger
    ...
    if 'use_only_authd' in api_conf:
        ...
        logger.warning(
            "'use_only_authd' option was deprecated on v4.3.0. Cyb3rhq Authd will always be used")
    ...
    try:
        ...
        print(f'Internal error when trying to start the Cyb3rhq API. {e}')
        sys.exit(1)
    finally:
