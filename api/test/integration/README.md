...
The API integration tests are used to verify that the API is working properly in a complete Cyb3rhq environment.
This environment is built using [`docker`](https://www.docker.com/).

The `cyb3rhq/api/test/integration` directory contains all the API integration tests files and directories used for the
...
The Cyb3rhq environment used to perform the API integration tests is built using `docker`.

This environment is composed of **12 docker containers**. These containers have the following components installed: 3
Cyb3rhq managers, that compose a Cyb3rhq cluster (1 master, 2 workers); 4 Cyb3rhq agents with the same version as the managers
forming the cluster; 4 Cyb3rhq agents with version 3.13.2 (old); and 1 HAProxy load balancer.

The Cyb3rhq version used for the managers and non-old agents is the one specified by the branch used to perform the API
integration tests.

The `docker-compose.yml` file used to deploy the environment is at `cyb3rhq/api/test/integration/env`. The `Dockerfile`,
...
The Cyb3rhq docker environment will have a different configuration depending on
the [`pytest` mark](https://docs.pytest.org/en/6.2.x/mark.html) used to run the tests with.

- If the `standalone` mark is specified, a Cyb3rhq environment with **1 manager and 12 agents** will be built (no cluster)
  .

- If the `cluster` mark is specified, a Cyb3rhq cluster setup with **3 managers and 12 agents** will be built.

- If **no mark** is specified, a Cyb3rhq cluster setup with **3 managers and 12 agents** will be built.

The following table shows how these marks must be used with the `pytest` command and the environment they build:

| Command                          | Environment                                          |  
|----------------------------------|------------------------------------------------------|
| `pytest TEST_NAME`               | Cyb3rhq cluster environment                            |  
| `pytest -m cluster TEST_NAME`    | Cyb3rhq cluster environment                            |
| `pytest -m standalone TEST_NAME` | Cyb3rhq environment with cluster disabled (standalone) | 
...
standalone** can only be passed in a Cyb3rhq environment with cluster disabled and cases with **only cluster** mark can
only be passed in a Cyb3rhq cluster environment.
...
These tests are used to check the proper functioning of a Cyb3rhq environment with RBAC configurations. The `conftest.py`
...
Every time a pull request is created in GitHub for the `cyb3rhq` repository, a battery of checks is performed in the CI
...
The `cyb3rhq/api/test/integration/mapping` directory contains the `integration_test_api_endpoints.json` file that
...
To perform a Cyb3rhq API integration test, we need a specific `python3` environment. This python environment includes the 
...
rootdir: /home/user/git/cyb3rhq/api/test/integration, inifile: pytest.ini
...
We can also use the `cyb3rhq/api/test/integration/run_tests.py` script. This script includes the possibility to collect a 
...
optional arguments:
  ...
  -m {both,standalone,cluster}, --mode {both,standalone,cluster}
                        Specify where to pass API integration tests. Run tests in both environments, standalone environment or Cyb3rhq cluster environment. Default "both".
...
at `cyb3rhq/api/test/integration/_test_results`. Containers' logs (`ossec.log`, `api.log` and `cluster.log`) are stored 
