...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R cyb3rhq:cyb3rhq /var/ossec/api
...
if [ $4 == "standalone" ]; then
  ...
  sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/scripts/cyb3rhq_apid.py
fi
...
cd /var/ossec/etc/shared && find -name merged.mg -exec chown cyb3rhq:cyb3rhq {} \; && cd /
...
if [ "$3" == "master" ]; then
  ...
  # Wait until Cyb3rhq API is ready
  elapsed_time=0
